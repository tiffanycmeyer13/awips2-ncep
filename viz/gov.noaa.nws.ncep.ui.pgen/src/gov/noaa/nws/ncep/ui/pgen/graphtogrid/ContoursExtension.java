...
public class ContoursExtension {
    ...
    private int[] npts = null;

    // Latitudes as [nlines][ npts[ line n ] ]
    private float[][] flat = null;

    // Longitudes as [nlines][ npts[ line n ] ]
    private float[][] flon = null;

    // Number of points on each contour line
    private int[] nOrigPts = null;

    // Smoothed latitudes in Grid coordinate
    private float[][] fi_orig = null;

    // Smoothed longitudes in Grid coordinate
    private float[][] fj_orig = null;

    // Number of points on each line
    private int[] nExtPts = null;

    // Smoothed and extended latitudes in Grid space
    private float[][] fi_ext = null;

    // Smoothed and extended latitudes in Grid space
    private float[][] fj_ext = null;

    // Value for each contour line
    private float[] value = null;

    // closeness for each line
    private int[] closed = null;

    // smoothing factor for each line
    private int[] ismth = null;
    ...
    /**
     * @param gtrans
     *            the ToordinateTransform to set
     */
    ...
    private void computeExtentions() {
        ...
        for (ContourLine cline : clines) {
            ...
            value[ii] = GraphToGrid.getValueForLabel(catmap,
        ...
        if (ln != null) {
            ...
            if (!ln.isClosedLine()) {
                ...
                newPt = extendLine(fj_orig[index][0], fi_orig[index][0],
                        fj_orig[index][1], fi_orig[index][1], kx, ky);
                ...
                newPt2 = extendLine(fj_orig[index][np - 1],
                        fi_orig[index][np - 1], fj_orig[index][np - 2],
                        fi_orig[index][np - 2], kx, ky);
     ...
     */
    private double[] extendLine(float px1, float py1, float px2, float py2,
            int kx, int ky) {
