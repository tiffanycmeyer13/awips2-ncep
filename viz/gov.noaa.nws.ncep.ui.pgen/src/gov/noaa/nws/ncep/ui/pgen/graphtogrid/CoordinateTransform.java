...
public class CoordinateTransform {
    ...
    private static final IUFStatusHandler statusHandler = UFStatus
            .getHandler(CoordinateTransform.class);

    /*
     * Configure the two files in the extension
     */
    private final String geogFilePath = PgenStaticDataProvider.getProvider()
            .getGeogFile().getAbsolutePath();

    private final String stationFilePath = PgenStaticDataProvider.getProvider()
            .getSfcStnFile().getAbsolutePath();

    /*
     * Max dimension for holding intersections defined in grphgd.cmn (this
     * allows for 1000x1000 grid)
     */
    ...
    /**
     * Set up the default values and find the math transformation and the
     * scaling factors to be used for coordinate translation.
     */
    private void findTransform() {

        /*
         * Default will the Mercator projection within US with a 63 by 28 grid.
         */
        ...
        try {
            ...
            try {
                ...
                CustomProjectionServiceImpl customProj = new CustomProjectionServiceImpl(
                        projection, garea, geogFilePath, stationFilePath);
                CoordinateReferenceSystem crs = customProj
                        .getCoordinateReferenceSystem();
                if (crs == null) {
                    statusHandler
                            .error("Failed to perform coordinate transform because of improper parameter "
                                    + "usage (e.g. a value out of range). Can't continue...");
                    return;
                }

                GeneralGridGeometry gridGeom = MapDescriptor
                        .createGridGeometry(crs, ll, ur);

                /*
                 * Find the math transformation between world coordinate and the
                 * native geometry.
                 */
                ...
                worldToGeneralGrid = dmtFactory.createConcatenatedTransform(m2c,
                        c2g);
                generalGridToWorld = dmtFactory.createConcatenatedTransform(g2c,
                        c2m);
    ...
    /**
     * Convert a set of points in map coordinate to the customized grid
     * coordinate - which scales coordinates between (1, 1) to (kx, ky) with
     * (1,1) at the lower left corner.
     * 
        ...
        if (numPts > 0 && worldToGeneralGrid != null) {
            ...
            if (outp != null && outp.length > 0) {
                ...
                for (int ii = 0; ii < numPts; ii++) {
                    ...
                    gridout[ii * 2 + 1] = (ysize - outp[ii * 2 + 1] - 0.5)
                            / yscale + 1;
                }
            }
    ...
    /**
     * Convert a set of points in the customized grid coordinate to map
     * coordinate. The input points are scaled between (1, 1) to (kx, ky) with
     * (1,1) at the lower left corner.
     * 
     * Note: the input should be organized as (x1, y1, x2, y2, ...) where x is
     * corresponding to longitude in map coordinate and y is corresponding to
     * latitude.
     */
    public double[] gridToWorld(double[] gridin) {
        ...
        if (numPts > 0 && generalGridToWorld != null) {
            ...
            /*
             * Scale the input from the input grid space to the native grid
             * geometry.
             */
            double[] outp = new double[numPts * 2];
            ...
            for (int ii = 0; ii < numPts; ii++) {
                ...
                outp[ii * 2 + 1] = ysize - (gridin[ii * 2 + 1] - 1) * yscale
                        - 0.5;
            }
    ...
    public Coordinate[] worldToGeneralGrid(Coordinate[] lonlat) {
        ...
        if (ptsout != null && ptsout.length > 0) {
            ...
            for (int ii = 0; ii < numPts; ii++) {
                gridout[ii] = new Coordinate(ptsout[ii * 2],
                        ptsout[ii * 2 + 1]);
            }
        }
