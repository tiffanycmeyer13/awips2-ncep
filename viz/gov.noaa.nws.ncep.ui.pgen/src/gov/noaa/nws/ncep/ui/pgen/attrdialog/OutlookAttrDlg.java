 ...
 * 01/12         582      Q. Zhou   Added NHC line/label group. Added flagLabel
 *                                  and flagAction and enable function. Fixed
 *                                  enable problem when dialog returns.
 * 03/12         599      Q. Zhou   Fixed selecting a outlook. need to enable
 *                                  btns from outlookType table. Fixed label
 *                                  text combo width.
...
public class OutlookAttrDlg extends AttrDlg implements IContours, ILine {
    ...
    private static enum ChkBox {
        COLOR, WIDTH, SMOOTH, CLOSE, FILL
    };

    // Symbol icons in the symbol drop-down menu
    private static final String[] SYMBOL_LIST = new String[] { "PAST_WX_09",
            "PRESENT_WX_065", "PRESENT_WX_073", "PRESENT_WX_075",
            "PRESENT_WX_073|PRESENT_WX_075", "PRESENT_WX_056", "PRESENT_WX_079",
            "PRESENT_WX_056|PRESENT_WX_079" };
    ...
    private final String defaultLinetype = "POINTED_ARROW";
    ...
    /**
     * Creates an outlook attribute dialog if the dialog does not exist and
    ...
    protected void initializeComponents() {
        ...
        outlookCombo.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                // Loop through current layer and see if there is a different
                // type of outlook.
                ...
                while (it.hasNext()) {
                    ...
                    if (adc instanceof Outlook && !((Outlook) adc)
                            .getOutlookType().equalsIgnoreCase(type)) {
                        warning = true;
                        break;
                    }
                }
                ...
                if (warning) {
                    MessageDialog msgDlg = new MessageDialog(
                            PlatformUI.getWorkbench().getActiveWorkbenchWindow()
                                    .getShell(),
                            "Warning!", null,
                            "There is at least one outlook with different type in the same layer!",
        ...
        // Container to hold 'Add-Line', 'Del-Line', 'Set-Cont', and 'Show-Cont'
        // buttons
        ...
        delLineBtn.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                if (drawingLayers.getSelectedDE() != null && de != null
                        && de instanceof Line
                        && de.getParent().getName().equalsIgnoreCase(
                    ...
                    if (oldOtlk.size() <= 1) {
                        ...
                        if (drawingLayers.getActiveLayer().getDrawables()
                                .contains(oldOtlk)) {
                            drawingLayers.removeElement(oldOtlk);
                        }
                    } else {
                        // to make the undo work, the entire outlook should be
                        // replaced.

                        // save the category of the de and mark the deleting
                        // Line by setting category to 'del'
                        ...
                        Iterator<DrawableElement> it = newOtlk
                                .createDEIterator();
                        while (it.hasNext()) {
                            DrawableElement tmpDe = it.next();
                            if (tmpDe instanceof Line && tmpDe.getPgenCategory()
                        ...
                        drawingLayers.replaceElement(oldOtlk, newOtlk);
        ...
        fmtBtn.addSelectionListener(new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                fmtDlg = new OutlookFormatDlg(
                        OutlookAttrDlg.this.getParentShell(),
                        OutlookAttrDlg.this, findOtlk());
                fmtDlg.open();
            }
        });
    ...
    private void createFillAttr() {
        ...
        chkBox[ChkBox.FILL.ordinal()]
                .setLayoutData(new GridData(CHK_WIDTH, CHK_HEIGHT + 5));
        chkBox[ChkBox.FILL.ordinal()]
                .addSelectionListener(new SelectionAdapter() {
    ...
    public void showContLines(Outlook ol) {
        ...
        while (it.hasNext()) {
            ...
            if (adc.getName().equalsIgnoreCase(Outlook.OUTLOOK_LINE_GROUP)) {
                ...
                for (int ii = 0; ii < lns.size() - 1; ii++) {
                    ...
                    pts.add(lns.get(ii).getPoints()
                            .get(lns.get(ii).getPoints().size() - 1));
                    pts.add(lns.get(ii + 1).getPoints().get(0));

                    Line vLn = new Line(null, lns.get(0).getColors(),
                            lns.get(0).getLineWidth(),
                            lns.get(0).getSizeScale(), false, false, pts,
                            lns.get(0).getSmoothFactor(), null, "Lines",
                            "LINE_DASHED_4");
        ...
        if (!vLines.isEmpty()) {
            drawingLayers.setGhostLine(vLines);
            mapEditor.refresh();
        }
    }
    ...
    public static Document readOutlookTbl() {
        ...
        if (outlookTbl == null) {
            try {
                String outlookTypeFile = PgenStaticDataProvider.getProvider()
    ...
    private List<String> getOutlookTypes() {
        ...
        List<String> otlkType = new ArrayList<>();
    ...
    /**
     * Load default settings from outlooksettings.xml Setting is a hash map with
     * outlook type and label string as keys.
     */
    private static void loadSettings() {
        ...
        String settingFile = PgenStaticDataProvider.getProvider()
                .getFileAbsolutePath(PgenStaticDataProvider.getProvider()
                        .getPgenLocalizationRoot() + "outlooksettings.xml");
    ...
    public void okPressed() {
        ...
        /*
         * Find the Outlook that contains the selected DE. Create a copy of it,
         * and update its attributes as well as the label/line attributes.
         */
        if (parentAdc != null && parentAdc.getParent() instanceof Outlook) {
            ...
            Iterator<AbstractDrawableComponent> iterator = oldOutlook
                    .getComponentIterator();
            ...
            while (iterator.hasNext()) {
                ...
                if (oadc.equals(parentAdc)) {
                    ...
                    Iterator<DrawableElement> deit = parentAdc
                            .createDEIterator();
                    ...
                    while (deit.hasNext()) {
                        ...
                        if (nde instanceof Line) {
                            ...
                            ((Text) nde)
                                    .setText(new String[] { this.getLblTxt() });
            ...
            /*
             * Update the outlook attributes and replace the old one with the
             * new outlook.
             */
            ...
            drawingLayers.replaceElement(oldOutlook, newOutlook);
    ...
    private boolean showMakeGrid(String type) {
        ...
        if (type != null) {
            ...
            String makegrid = OutlookAttrDlg.readOutlookTbl()
                    .selectSingleNode(xpath).valueOf("@makegrid");
            if (makegrid != null && !makegrid.isEmpty()
                    && makegrid.equalsIgnoreCase("false")) {
                ret = false;
            }
        }
    ...
    private boolean showLabel(String type) {
        ...
        if (type != null) {
            ...
            String xpath = OutlookAttrDlg.OTLK_XPATH + "[@name='" + type + "']";
            String makegrid = OutlookAttrDlg.readOutlookTbl()
                    .selectSingleNode(xpath).valueOf("@flagLabel");
            if (makegrid != null && !makegrid.isEmpty()
                    && makegrid.equalsIgnoreCase("false")) {
                ret = false;
            }
    ...
    private boolean showAction(String type) {
        ...
        if (type != null) {
            ...
            String xpath = OutlookAttrDlg.OTLK_XPATH + "[@name='" + type + "']";
            String makegrid = OutlookAttrDlg.readOutlookTbl()
                    .selectSingleNode(xpath).valueOf("@flagAction");
            if (makegrid != null && !makegrid.isEmpty()
                    && makegrid.equalsIgnoreCase("false")) {
                ret = false;
            }
    ...
    private boolean setFromLineFlag(String type) {
        ...
        if (type != null) {
            ...
            String xpath = OutlookAttrDlg.OTLK_XPATH + "[@name='" + type + "']";
            String makegrid = OutlookAttrDlg.readOutlookTbl()
                    .selectSingleNode(xpath).valueOf("@fromline");
            if (makegrid != null && !makegrid.isEmpty()
                    && makegrid.equalsIgnoreCase("true")) {
                ret = true;
            }
        }
    ...
    /**
     * Find the outlook to format. 1. check if an outlook is selected 2. get the
     * first outlook in the layer 3. null
     *
     * @return
     */
