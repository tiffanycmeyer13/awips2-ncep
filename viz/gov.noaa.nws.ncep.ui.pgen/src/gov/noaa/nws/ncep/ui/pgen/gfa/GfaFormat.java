 ...
 * 06/11                     J. Wu        Apply computational coordinate and
 *                                        clipped smears before snapping.
 * 06/11                     J. Wu        Fixed missing CIG type in
 *                                        findGfaSubTypes().
 * 07/11                     J. Wu        Removed the pre-clipping before
 *                                        snapping.
 * 07/11         ?           B. Yin       Added FRZL
 * 05/12         808         J. Wu        Update vor text for airmets.
 * 06/12         594         J. Wu        TTR393 - fixed invalid polygons from
 *                                        clipping.
 * 08/12         610         B. Yin       Remove M_FZLVL outlooks
 * 08/12         859         J. Wu        Fixed smearing when snapshots is too
 *                                        small.
 * 11/12         911         J. Wu        TTR 652 - Validate GFA before
 *                                        smearing, warn the user and exclude
 *                                        invalid GFAs from FROM. Also, prevent
...
public class GfaFormat {
    ...
    PgenResourceList drawingLayers;
    ...
    public GfaFormat(PgenResourceList drawingLayers) {
    ...
    public void formatAllPressed() {
        ...
        if (drawingLayers != null) {
            validateAllGfas();
            for (Product p : drawingLayers.getProducts()) {
    ...
    public void formatLayerPressed() {
        ...
        if (drawingLayers != null) {
            validateActiveGfas();
            Layer layer = drawingLayers.getActiveLayer();
            formatLayer(layer, false);
        }
    }
    ...
    private void formatLayer(Layer layer, boolean checkTag) {
        ...
        for (AbstractDrawableComponent adc : layer.getDrawables()) {
            ...
            if (checkTag) {
                if (de != null
                        && !isSameHazardAndTag(((Gfa) adc), ((Gfa) de))) {
                    continue;
                }
            }
            ...
            if (!((((Gfa) adc).getGfaHazard().equalsIgnoreCase("M_FZLVL")
                    ...
                    && (Gfa.getHourMinInt(((Gfa) adc).getGfaFcstHr())[0]
                            + Gfa.getHourMinInt(((Gfa) adc).getGfaFcstHr())[1]
                                    / 60.0) > 6)) {
                oldList.add(adc);
            }

            if (((Gfa) adc).isSnapshot() && !((((Gfa) adc).getGfaHazard()
                    .equalsIgnoreCase("M_FZLVL")
                    || ((Gfa) adc).getGfaHazard().equalsIgnoreCase("LLWS"))
                    && (Gfa.getHourMinInt(((Gfa) adc).getGfaFcstHr())[0]
                            + Gfa.getHourMinInt(((Gfa) adc).getGfaFcstHr())[1]
                                    / 60.0) > 6)) {
    ...
    protected void createSmears(List<AbstractDrawableComponent> list) {
        /*
         * Put snapshots into a tree set and sort them by fcstHr in ascending
         * order.
         */
        TreeSet<Gfa> tree = new TreeSet<>(new SnapshotComparator());
        for (AbstractDrawableComponent adc : list) {
            if (((Gfa) adc).getGfaHazard().equalsIgnoreCase("FZLVL")) {
                continue;
            }
            if (((Gfa) adc).isFormat()) {
                tree.add((Gfa) adc);
         ...
         *
         * Each pair has a list of snapshots and a forecast hour "A-B": A -
         * smallest forecast hour of all snapshots in the pair B - largest
         * forecast hour in the pai). Snapshots in each pair has the same hazard
         * type, tag number, and desk.
         */
        TreeSet<FcstHrListPair> splittedSS = splitSnapshots(tree);

        // Get the GFA international bounds
        Coordinate[] intlBnds = GfaClip.getInstance().getFaInternationalBound()
                .getGeometryN(0).getCoordinates();
        ArrayList<Coordinate> intlBndsPts = new ArrayList<>();
         ...
         */
        ArrayList<ArrayList<Gfa>> listOfLists = new ArrayList<>();
        for (FcstHrListPair pair : splittedSS) {
            ...
            ArrayList<Gfa> toRemove = new ArrayList<>();
            ...
            for (int ii = 0; ii < pair.list.size() - 1; ii++) {
                ...
                if (g.getGfaFcstHr().equals(gNext.getGfaFcstHr())) {
                    ...
                    ArrayList<Gfa> l = new ArrayList<>();
                    l.add(g);
                    l.add(gNext);

                    ArrayList<Coordinate> shrinkWrapped = createModifiedHull(
                            getGeometryFactory(), l);
            ...
            ArrayList<Polygon> toUnite = new ArrayList<>();
             ...
             * Perform union
             *
             * A JTS polygon will have its first/last point the same. We remove
             * the last point for snapping.
             */
             ...
             * to eliminate the difficulty and complexity to find snap points
             * for those points outside of the international boundary - snap
             * points are very sparse or none when moving away from the
             * international boundary.
             */
            Geometry intlBoundInGrid = GfaClip.getInstance()
                    .getFaInternationalBoundInGrid();

            ArrayList<Coordinate> pointsInGrid = PgenUtil
                    .latlonToGrid(coordinates);

            Geometry smearPolyInGrid = GfaClip.getInstance()
                    .pointsToGeometry(pointsInGrid);
            ...
            if (clipAgstIntlBnd == null
                    || clipAgstIntlBnd.getNumGeometries() <= 0
             ...
             *
             * do not clip against the international bound before snapping - may
             * cause unwanted kinks and hard to keep the bound intact.
             */
            ArrayList<Coordinate> snapped = GfaSnap.getInstance()
                    .snapPolygon(coordinates);

            // Create a GFA smear
            Gfa smear = pair.list.get(0);
            smear = new Gfa(snapped, smear.getGfaHazard(), pair.fcstHr,
                    smear.getGfaTag(), smear.getGfaDesk(),
                    smear.getGfaIssueType(), type, values);
            ...
            ArrayList<Gfa> clippedWithRegions = GfaClip.getInstance()
                    .clipFARegions(smear, pair.original);

            if (clippedWithRegions == null || clippedWithRegions.size() == 0) {
                continue;
            }

            /*
             * Clipping may result in invalid polygons, when intersection points
             * of the smear with the international bounds are snapped....
             */
            for (Gfa gg : clippedWithRegions) {
                if (!(gg.toPolygon().isValid())) {
                    ...
                    for (int ii = 0; ii < gg.getPoints().size(); ii++) {
                        int jj = ii + 1;
                        if (jj == gg.getPoints().size()) {
                            jj = 0;
                        }

                        double distance = GfaSnap.getInstance().distance(
                                gg.getPoints().get(ii), gg.getPoints().get(jj));
                        if ((distance
                                / PgenUtil.NM2M) <= GfaSnap.CLUSTER_DIST) {
            ...
            /*
             * Add the new smear and its associated snapshots as attributes to
             * the list for further processing.
             */
            clippedWithRegions.get(0).addAttribute("TEMP_SMEAR", smear);

            for (Gfa g : clippedWithRegions) {
                // just to pass to the next for loop
                g.addAttribute("TEMP_SMEAR", smear);

                if (smear.isOutlook()) {
                    // pass to GfaRules
                    g.addAttribute("OUTLOOKS", clippedWithRegions);
                } else {
                    // pass to GfaRules
                    g.addAttribute("AIRMETS", clippedWithRegions);
                }
            ...
            // Temp save, then apply rules after all the smears and outlooks are
            // created
        ...
        for (ArrayList<Gfa> clippedGroup : listOfLists) {
            ...
            Gfa smear = (clippedGroup.get(0).getAttribute("TEMP_SMEAR",
                    Gfa.class));

            clippedGroup.get(0).removeAttribute("TEMP_SMEAR");

            FcstHrListPair pair = clippedGroup.get(0).getAttribute("PAIR",
            ...
            GfaWorstAttr.apply(smear, clippedGroup, pair.original,
                    pair.canceled);
    ...
    /**
     * A Class to hold a list of snapshots and a forecast hour ("A-B", A -
     * smallest forecast hour of all snapshots in the pair B - largest forecast
     * hour in the pair). Snapshots in each pair has the same hazard type, tag
     * number, and desk.
     *
     * Note: (1) a second list is used for future reference to hold all ORIGINAL
     * snapshots that are used to create the smear. (2) a third list is used for
     * future references to hold all REMOVED snapshots (CANceled snapshots which
     * are not used to generate smears, but they have the same hazard type and
     * tag/desk as this pair).
     *
     */
    class FcstHrListPair implements Comparable<FcstHrListPair> {
        ...
        ArrayList<Gfa> all; // including all snapshot with same type, tag, and
                            // desk.

        public FcstHrListPair(String fcstHr, ArrayList<Gfa> list,
                ArrayList<Gfa> canceled, ArrayList<Gfa> allss) {
            this.fcstHr = fcstHr;
            this.list = list;
     ...
     * number, and desk.
     *
     * Note: a second list is used to hold all ORIGINAL snapshots for future
     * references.
     *
     * @param tree
     * @return
    ...
    private TreeSet<FcstHrListPair> splitSnapshots(TreeSet<Gfa> tree) {
        TreeSet<FcstHrListPair> pairs = new TreeSet<>();
        ArrayList<Gfa> list1 = new ArrayList<>(); // 0-6 smear
        ArrayList<Gfa> list2 = new ArrayList<>(); // 6-12 outlook
        ArrayList<Gfa> list3 = new ArrayList<>(); // all snapshots

        /*
         * First splits a tree set of GFA snapshots into two lists. "0-?" for
         * snapshots with forecast hour <= 6 "6-?" for snapshots with forecast
         * hour >= 6
         */
        for (Gfa gfa : tree) {
        ...
        HashMap<String, ArrayList<Gfa>> map = new HashMap<>();
        splitByHazardTag(list1, map);
        splitByHazardTag(list2, map);

        // split list3 by hazard type, tag, and desk
        HashMap<String, ArrayList<Gfa>> mapall = new HashMap<>();
        splitByHazardTag(list3, mapall);

        /*
         * Create FcstHrListPair for each list of snapshots (key is the forecast
         * hour of the group plus hazard type, tag number, and desk.
         *
         * Note: (1) "6-6" needs special attention (2) canceled snapshots needs
         * special rules.
         */
        ArrayList<ArrayList<Gfa>> all66 = new ArrayList<>();
        for (String key : map.keySet()) {
            ...
            if (!l.isEmpty()) {
                ...
                if ("6-6".equals(fcst)) {
                    ...
                    FcstHrListPair pair = new FcstHrListPair(fcst, l,
                            canceledSS, allss);
        ...
        for (ArrayList<Gfa> l66 : all66) {
            ...
            label: for (ArrayList<Gfa> l : map.values()) {
                if (l66 == l || isOutlook(determineFcst(l))) {
                    continue;
                }
                for (Gfa g : l) {
                    if (g == gfa) {
            ...
            if (!found) {
                ...
                for (String key : map.keySet()) {
                    if (key.endsWith(gfa.getGfaHazard() + gfa.getGfaTag()
                            + gfa.getGfaDesk())
    ...
    /**
     * Build a map for a list of GFAs snapshots. The key is the forecast hour of
     * the list (("A-B", A and B are the smallest and largest forecast hours of
     * all snapshots in the list, respectively) plus a Gfa's hazard type, tag
     * number, and desk. The value is a new list of snapshots with the same key.
     *
     * @param list
     * @param map
     * @return
     */
    private void splitByHazardTag(ArrayList<Gfa> list,
            HashMap<String, ArrayList<Gfa>> map) {

        for (Gfa g : list) {

            String key = determineFcst(list) + g.getGfaHazard() + g.getGfaTag()
                    + g.getGfaDesk();
    ...
    /**
     * Make a airmet/outlook forecast hour from a list of snapshots. Note: the
     * snapshots are assumed to be sorted by their forecast hours in an
     * ascending order.
     *
     */
    ...
    /**
     * Check if a forecast hour qualifies as an outlook ("A-B", A >= 6 and B
     * exists and is assumed >= 6 as well).
     */
    ...
    /**
     * Find canceled snapshots that should be removed, but when all of them are
     * canceled, all of them should be kept.
     *
     * Note: (1) the snapshots that should be removed are removed from the
     * original list. (2) a list of removed "CAN" snapshots are returned in a
     * new list. If all are "CAN" snapshots and should be kept, then the
     * returned "CAN" list is empty.
     *
    ...
    private ArrayList<Gfa> findCancelled(ArrayList<Gfa> list) {
        ...
        ArrayList<Gfa> cancelled = new ArrayList<>();
        for (Gfa gfa : list) {
     ...
     */
    private ArrayList<Coordinate> createModifiedHull(GeometryFactory gf,
            ArrayList<Gfa> list) {
         ...
         * (shrink wrap smear)
         */
        ArrayList<Coordinate> hullCoordinatesList = shrinkWrapSmear(gf, rings,
                hullCoordinates);
     ...
     */
    private ArrayList<Coordinate> shrinkWrapSmear(GeometryFactory gf,
            LinearRing[] rings, Coordinate[] hullCoordinates) {

        int i;
        ArrayList<Coordinate> hullCoordinatesList = new ArrayList<>();

        for (i = 0; i < hullCoordinates.length - 1; i++) {
            Coordinate c = hullCoordinates[i];
            Coordinate cNext = (i + 1) == hullCoordinates.length - 1
                    ? hullCoordinates[0] : hullCoordinates[i + 1];
            ...
            if (!hullCoordinatesList.contains(c)) {
                hullCoordinatesList.add(c);
            }
            ...
            for (LinearRing ring : rings) {
                if (ring.contains(p) && ring.contains(pNext)) {
                    ...
                    if (Math.abs(indexNext - index) <= 1) {
                        continue;
                    }

                    indexNext = (indexNext > index) ? indexNext
                            : indexNext + ringCoordinates.length;

                    // insert intermediate points
                    for (int ii = index; ii < indexNext; ii++) {
                        Coordinate toInsert = ringCoordinates[ii
    ...
    /**
     * Find the worst case top, bottom, FZL top, and FZL bottom from a list of
     * GFA snapshots
     *
     * @param list
     *            list of GFA snapshots
     * @return
     */
    public static HashMap<String, String> findGfaTopBots(ArrayList<Gfa> list) {
        ...
        Comparator<String> comp = new Comparator<String>() {
            ...
            public int compare(String s1, String s2) {
                if (s1.equals(s2)) {
                    return 0;
                } else if ("SFC".equalsIgnoreCase(s1)
                        && "FZL".equalsIgnoreCase(s2)) {
                    return -1;
                } else if ("FZL".equalsIgnoreCase(s1)
                        && "SFC".equalsIgnoreCase(s2)) {
                    return 1;
                } else if ("SFC".equalsIgnoreCase(s1)
                        || "FZL".equalsIgnoreCase(s1)) {
                    return -1;
                } else if ("SFC".equalsIgnoreCase(s2)
                        || "FZL".equalsIgnoreCase(s2)) {
                    return 1;
                }
         ...
         */
        TreeSet<String> tops = new TreeSet<>(comp);
        TreeSet<String> bottoms = new TreeSet<>(comp);
        for (Gfa gfa : list) {
    ...
    /**
     * Build a sorted string from all GFA subtypes presented in a list of GFA
     * snapshots
     *
     * @param list
     *            list of GFA snapshots
    ...
    public static String findGfaSubTypes(ArrayList<Gfa> list) {
        ...
        TreeSet<String> visTypes = new TreeSet<>(new Comp());
        for (Gfa gfa : list) {
            if (gfa.getGfaType() != null && !gfa.getGfaType().isEmpty()) {

                String[] s = gfa.getGfaType().replace("/VIS", ":VIS")
                        .split(":");
                int ii = 0;
    ...
    private class SnapshotComparator implements Comparator<Gfa> {
        ...
        public int compare(Gfa g1, Gfa g2) {
            if (g1 == null || g2 == null || !g1.isSnapshot()
                    || !g2.isSnapshot()) {
                return -1;
            }
            ...
            try {
                ...
                if (d1 < d2 || Math.abs(d2 - d1) <= 0.0001) {
                    return -1;
                } else {
                    return 1;
                }
    ...
    private List<Gfa> formatFrzl(Layer layer, boolean checkTag) {
        ...
        if (smears.isEmpty()) {
            ...
            return new FrzlFormatter(smears).format();
        }
    }
    ...
    public static void writePoints(ArrayList<Coordinate> snp, String filename) {
        ...
        try {
            ...
            for (Coordinate cc : snp) {
                String c = new String("<Point Lat=\"" + cc.y + "\" Lon=\" "
                        + cc.x + " \"/>\n");
                output.write(c);
            }
    ...
    /*
     * Validate Gfas in all activities and warn for invalid Gfas in the
     * resource.
     */
    private void validateAllGfas() {

        PgenResource psrc = PgenSession.getInstance().getCurrentResource();
        StringBuilder msg = new StringBuilder();
        msg.append(
                "Warning: The following Gfas are invalid.  Please correct them or they will be ");
            ...
            for (Layer layer : prd.getLayers()) {
                for (AbstractDrawableComponent adc : layer.getDrawables()) {
                    if (adc instanceof Gfa && !((Gfa) adc).isValid()) {
                        nn++;
                        Gfa gg = (Gfa) adc;
                        msg.append(prd.getName() + "\t" + prd.getType() + "\t"
                                + layer.getName() + "\t" + gg.getGfaHazard()
                                + "," + gg.getGfaFcstHr() + "," + gg.getGfaTag()
                                + gg.getGfaDesk() + "\n");
                    }
                }
            }
    ...
    private void validateActiveGfas() {
        ...
        Layer curLayer = PgenSession.getInstance().getCurrentResource()
                .getActiveLayer();
        StringBuilder msg = new StringBuilder();
        msg.append(
                "Warning: The following Gfas are invalid.  Please correct them or they will be \n");
    ...
    /*
     * Find a list of all Gfa snapshots with same type/tag/desk as the given
     * one.
     */
    private ArrayList<Gfa> findFullList(Gfa gfa,
            HashMap<String, ArrayList<Gfa>> mapAll) {
        ArrayList<Gfa> fullList = new ArrayList<>();
        if (mapAll != null && mapAll.size() > 0) {
            String ttd = gfa.getGfaHazard() + gfa.getGfaTag()
                    + gfa.getGfaDesk();
