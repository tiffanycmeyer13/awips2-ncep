...
public class PgenDeletePart extends PgenSelectingTool {
    ...
    public void deactivateTool() {
        ...
        if (delPartHandler != null) {
            ((PgenDelPartHandler) delPartHandler).cleanup();
        }
    ...
    public IInputHandler getMouseHandler() {
        ...
        if (this.delPartHandler == null) {
            ...
            this.delPartHandler = new PgenDelPartHandler(this, mapEditor,
    ...
    public class PgenDelPartHandler extends PgenSelectHandler {
        ...
        private final Symbol DOT = new Symbol(null, new Color[] { Color.RED },
                1.0f, 7.5, false, null, "Marker", "DOT");

        OperationFilter delPartFilter = new OperationFilter(
         ...
         * @param attrDlg
         */
        public PgenDelPartHandler(AbstractPgenTool tool,
            ...
            if (button == 1) {
                ...
                if (drawingLayers.getSelectedDE() == null) {
                    ...
                    DrawableElement elSelected = drawingLayers
                            .getNearestElement(loc, delPartFilter);
                    ...
                    lil = getLil(loc, gf, drawingLayers.getSelectedDE());
                    if (lil == null) {
                        point2 = null;
                        return false;
                    }
                    ...
                    if (screenPt.distance(screen1) > TOL) { // not close enough
                                                            // to line
                        point1 = null;
                        return false;
                    }
                    ...
                    drawingLayers.setGhostLine(DOT);
                    ptSelected = true;

                } else {

                    // select second point
                    Line des = (Line) drawingLayers.getSelectedDE();
                    ...
                    /*
                     * if clicked close enough to line, save this 2nd point, and
                     * delete proper segment
                     */
                    LinearLocation linloc2 = lil.project(screenPt);
                    Coordinate screen2 = lil.extractPoint(linloc2);
                    if (screenPt.distance(screen2) > TOL) { // not close enough
                                                            // to line
                        point2 = null;
                        return false;
                    }
                    ...
                    if (des.getParent() instanceof ContourLine) {
                        Contours oldContours = (Contours) des.getParent()
                                .getParent();
                        Contours newContours = oldContours.split(
                                (ContourLine) des.getParent(), point1, point2);

                        drawingLayers.replaceElement(oldContours, newContours);
                    } else {
                        drawingLayers.deleteElementPart(des, point1, point2);
                    }
        ...
        /**
         * creates an array of coordinates representing a line in screen
         * coordinates after closing and smoothing (if necessary) a line given
        ...
        private LocationIndexedLine getLil(Coordinate loc, GeometryFactory gf,
            ...
            if (elem instanceof ILine && !(elem instanceof WatchBox)) {
                ...
                Coordinate[] coords = preprocessLine(
                        ((ILine) elem).getSmoothFactor(),
                        ((ILine) elem).isClosedLine(),
                        ((IMultiPoint) elem).getLinePoints());
