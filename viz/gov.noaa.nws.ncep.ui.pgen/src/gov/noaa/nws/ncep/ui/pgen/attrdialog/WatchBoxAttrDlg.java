 ...
 * 03/10         231      Archana   Altered the dialog for watch-box to display
 *                                  only buttons showing the selected colors
 *                                  instead of displaying the complete color
 *                                  matrix. Altered the layout of the composite
 *                                  to align the color selection button
 *                                  correctly.
...
public class WatchBoxAttrDlg extends AttrDlg implements IWatchBox {
    ...
    private static final String[] SYMBOL_LIST = new String[] { "PLUS_SIGN",
            "TRIANGLE", "UP_ARROW", "SMALL_X", "BOX", "OCTAGON" };
    ...
    public void setAttrForDlg(IAttribute iattr) {
        ...
        if (iattr instanceof IWatchBox) {
            ...
            if (fillClr != null) {
                symbolColor.setColorValue(new RGB(fillClr.getRed(),
    ...
    private void createWidthAttr() {
        ...
        widthSlider.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                widthText.setText(
                        new Float(widthSlider.getSelection() / 10.).toString());
            }
        });
        ...
        widthText.addKeyListener(new KeyAdapter() {
            ...
            public void keyReleased(KeyEvent e) {
                ...
                try {
                    ...
                    // lineWidthText.setToolTipText( "Only integer values
                    // between 1 and 10 are accepted." );
                }
            }
        });
    ...
    private void createSizeAttr() {
        ...
        sizeSlider.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                sizeText.setText(
                        new Float(sizeSlider.getSelection() / 10.).toString());
            }
        });
            ...
            Display.getDefault().asyncExec(new Runnable() {
                ...
                public void run() {
                    ...
                    if (!(shell == null || shell.isDisposed())) {
                        ...
                        if (infoDlg == null) {
                            infoDlg = WatchInfoDlg.getInstance(
                                    WatchBoxAttrDlg.this.getParentShell(),
                                    INSTANCE);
                        }
    ...
    public boolean close() {
        this.drawingLayers.removeSelected();
        wbTool = null;
        if (infoDlg != null) {
            infoDlg.close();
        }
    ...
    public Color getFillColor() {
        if (fillBtn.getSelection()) {
            return new java.awt.Color(symbolColor.getColorValue().red,
                    symbolColor.getColorValue().green,
                    symbolColor.getColorValue().blue);
        } else {
            return null;
        }
    }
    ...
    public WatchShape getWatchBoxShape() {
        ...
        if (nsBtn.getSelection()) {
            return WatchShape.NS;
        } else if (ewBtn.getSelection()) {
            return WatchShape.EW;
        } else if (esolBtn.getSelection()) {
            return WatchShape.ESOL;
        } else {
            return null;
        }
        ...
        if (adcList != null && !adcList.isEmpty()) {
            ...
            for (AbstractDrawableComponent adc : adcList) {
                ...
                if (el != null) {
                    ...
                    if (el.getWatchBoxShape() != this.getWatchBoxShape()) {
                        ArrayList<Station> anchorsInPoly = PgenWatchBoxDrawingTool
                                .getAnchorsInPoly(mapEditor,
                                        WatchBox.generateWatchBoxPts(
                                                this.getWatchBoxShape(),
                                                wb.getHalfWidth(),
                                                wb.getPoints().get(0),
                                                wb.getPoints().get(5)));
                        if (anchorsInPoly != null && !anchorsInPoly.isEmpty()) {
                            DECollection dec = new DrawableElementFactory()
                                    .createWatchBox("Met", "Watch",
                                            this.getWatchBoxShape(),
                                            wb.getPoints().get(0),
                                            wb.getPoints().get(5),
                                            anchorsInPoly, this);
                            if (dec != null) {
                                wb.setLinePoints(((WatchBox) dec.getPrimaryDE())
                                        .getPoints());
                                wb.setAnchors(
                                        ((WatchBox) dec.getPrimaryDE())
                                                .getAnchors()[0],
                                        ((WatchBox) dec.getPrimaryDE())
                                                .getAnchors()[1]);
                                updateShape = true;
                    ...
                    // if for any reason, shape cannot be changed, set the shape
                    // bake to the original.
                    if (!updateShape) {
    ...
    public void createButtonsForButtonBar(Composite parent) {
        ...
        this.getButton(IDialogConstants.CANCEL_ID)
                .setLayoutData(new GridData(90, 30));
