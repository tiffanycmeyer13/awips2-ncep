...
public class WatchCoordDlg extends CaveJFACEDialog {
     ...
     * @return
     */
    public static WatchCoordDlg getInstance(Shell parShell,
    ...
    public void okPressed() {
        ...
        if (!stormBtn.getSelection() && !tornadoBtn.getSelection()) {
            ...
            dirPath = PgenUtil.getPgenOprDirectory() + File.separator + pd1
                    + File.separator + "prod" + File.separator + "text"
                    + File.separator;
            updateWatch();

            String label = "WWC_WCL.xml";
            dataURI = wbDlg.getWatchBox().storeProduct(label);
            openWCCDlg();
        }
    }
    ...
    private void openWCCDlg() {
        ...
        WatchWCCDlg wccDlg = new WatchWCCDlg(PlatformUI.getWorkbench()
    ...
    private void openWCLDlg() {
        ...
        for (String state : wbDlg.getWatchBox().getStates()) {
            wclText += PgenToolUtils.wrapWatchText(wbDlg.getWatchBox()
                    .createCountyInfo(state, getExpirationTime()), LINE_LEN);
            wclText += "$$\n\n\n";
        }
