...
public class PgenVolcanoCreateTool extends AbstractPgenTool {
    ...
    protected void activateTool() {
        ...
        /*
         * 2010-04-14: one Product contains ONLY one Volcano; use a NEW Volcano
         * Product Center for another Volcano
         */
        if (isUsedVolProd()) {
            openConfirmBox(
                    "Please start with Volcano Activity to create a new volcano.");
            return;
        }
        ...
        VolcanoCreateDlg attrDlg = VolcanoCreateDlg.getInstance(PlatformUI
    ...
    /**
     * check if the Product already has a Volcano to ensure there is ONLY one
     * Volcano in a Product Center
     *
    ...
    private boolean isUsedVolProd() {
         ...
         *
         * changed to: if prod in VOL_PROD_MAP, but the Volcano is deleted then
         * this Product can be re-used
         */
        if (VaaInfo.VOL_PROD_MAP.containsValue(prod)) {
            ...
            for (Layer lyr : lyrList) {
                ...
                if (list != null) {
                    for (AbstractDrawableComponent adc : list) {
                        // a Volcano exists
                        if (adc != null && VaaInfo.PGEN_TYPE_VOLCANO
                                .equals(adc.getPgenType())) {
                            return true;
                        }

                        if (adc != null && VaaInfo.PGEN_TYEP_CLOUD
                                .equals(adc.getPgenType())) {
                            isCloudExist = true;
                        }
                    }
            ...
            /*
             * if Volcano is deleted but Ash Clouds still remain then user is
             * informed so clouds can be deleted or reused.
             */
            if (isCloudExist) {
                openConfirmBox(
                        "Not Deleted Ash Clouds of the Deleted Volcano Should Be\n"
    ...
    private void openConfirmBox(String msg) {
        MessageDialog confirmDlg = new MessageDialog(
                ...
                "Message", null, msg, MessageDialog.INFORMATION,
                new String[] { "OK" }, 0);
