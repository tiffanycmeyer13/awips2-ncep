...
public class ContourLine extends DECollection {
     ...
     * public constructor
     */
    public ContourLine(ArrayList<Coordinate> linePoints, boolean closed,
            String[] text) {
        super("ContourLine");
        ...
        for (String str : text) {
            ...
            Text lbl = new Text(null, "Courier", 14.0f,
                    ...
                    DisplayType.NORMAL, PgenConstant.CATEGORY_TEXT,
    ...
    public ContourLine(ArrayList<Coordinate> linePoints, boolean closed,
        ...
        /*
         * Create at least one label. If nlabels = 0, set the "hide" flag to
         * true so the label won't be displayed.
         */
        boolean hide = false;
        if (nlabels == 0) {
            hide = true;
        ...
        for (int ii = 0; ii < nlabels; ii++) {
            ...
            Text lbl = new Text(null, "Courier", 14.0f,
                    TextJustification.CENTER, null, 0.0,
                    TextRotation.SCREEN_RELATIVE, labelString,
                    FontStyle.REGULAR, Color.GREEN, 0, 0, true,
                    DisplayType.NORMAL, PgenConstant.CATEGORY_TEXT,
                    PgenConstant.TYPE_GENERAL_TEXT);
    ...
    /**
     * Get the label string of the contour line. Note: all the labels on a
     * contour line have the same text string.
     */
    ...
    /**
     * Updates the label string for the contour line. Note: all the labels on a
     * contour line have the same text string.
     */
    ...
    public void updateNumOfLabels(int nlabels) {
        ...
        if (getNumOfLabels() > 0) {
            /*
             * Hide labels if nlabels is 0. Otherwise, create "nlabels" from
             * existing ones.
             */
            if (nlabels == 0) {
                for (Text lbl : getLabels()) {
        ...
        // Set number of labels to the new value.
    ...
    /**
     * Remove a part from the contour line - older version, using only points on
     * line.
     */
    ...
    public ArrayList<ContourLine> split(Coordinate start, Coordinate end) {
        ...
        List<ArrayList<Coordinate>> newLines = PgenUtil.deleteLinePart(pts,
                closed, start, end);
    ...
    private ArrayList<Line> removePartFromClosedLine(Line element, int pt1Index,
        ...
        if (pt2Index - pt1Index
            ...
            element1.getPoints()
                    .addAll(points.subList(pt2Index, points.size()));
