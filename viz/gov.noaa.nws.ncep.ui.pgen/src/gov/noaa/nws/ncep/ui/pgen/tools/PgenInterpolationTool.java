 ...
 * 01/15         5413     B. Yin      Clear status when de-activate.
...
public class PgenInterpolationTool extends AbstractPgenDrawingTool {
     ...
     *
     */
    private enum SELECT_STATUS {
        START, SELECTED_1, VERIFIED_1, SELECTED_2, VERIFIED_2
    };
    ...
    /**
     * Constructor. Sets current status to START and creates a symbol to use for
     * verified elements
     */
    public PgenInterpolationTool() {
        ...
        verifySymbol = new Symbol(null, new Color[] { new Color(255, 0, 0) },
                1.0f, 1.0, false, null, "Marker", "FILLED_BOX");
    }
    ...
    public class PgenInterpHandler extends InputHandlerDefaultImpl {
        ...
        OperationFilter interpFilter = new OperationFilter(
        ...
        public boolean handleMouseDown(int anX, int aY, int button) {
            ...
            selectedEls = drawingLayers.getAllSelected();
            ...
            if (button == 1) {
                ...
                case START:
                    /*
                     * If the nearest element is valid, set it as a "selected"
                     * element
                     */
                    DrawableElement el1 = drawingLayers.getNearestElement(loc,
                            interpFilter);
                    if (selectionIsValid(el1)) {
                        drawingLayers.setSelected(el1);
                ...
                case SELECTED_1:
                    /*
                     * register the verify symbol with this element to show it
                     * in "verify" mode
                     */
                    drawingLayers.registerSelectedSymbol(selectedEls.get(0),
                            verifySymbol);
                ...
                case VERIFIED_1:
                    /*
                     * If the nearest element is of same category and closed
                     * status AND is different than the first "selected"
                     * element, then set this also as a "selected" element
                     */
                    DrawableElement el2 = drawingLayers.getNearestElement(loc,
                            interpFilter);
                    DrawableElement first = selectedEls.get(0).getPrimaryDE();
                    if (comparisonIsValid(el2, first)) {
                        drawingLayers.addSelected(el2);
                ...
                case SELECTED_2:
                    /*
                     * register the verify symbol with the second element to
                     * show it in "verify" mode
                     */
                    drawingLayers.registerSelectedSymbol(selectedEls.get(1),
                            verifySymbol);
        ...
        public boolean handleMouseUp(int x, int y, int button) {
            ...
            if (button == 3) {
                ...
                case SELECTED_2:
                     ...
                     */
                    drawingLayers.removeSelected(selectedEls.get(1));
                    status = SELECT_STATUS.VERIFIED_1;
                    break;
                case VERIFIED_2:
                    /*
    ...
    /**
     * Compares a new element with an original one to make sure that both are of
     * the same Pgen Category AND that both are open or both are closed
     *
     ...
     */
    public boolean comparisonIsValid(DrawableElement el2,
            DrawableElement orig) {

        if (orig == null || el2 == null) {
            return false;
        }
        if (orig == el2) {
            return false;
        ...
        if (orig instanceof Gfa) {
            ...
            if (el2 instanceof Gfa
                    && ((Gfa) el2).getGfaHazard()
                            .equals(((Gfa) orig).getGfaHazard())
                    && ((Gfa) el2).getGfaTag().equals(((Gfa) orig).getGfaTag())
                    && ((Gfa) el2).getGfaDesk()
                            .equals(((Gfa) orig).getGfaDesk())) {
                return true;
            } else {
                return false;
            }
    ...
    public boolean selectionIsValid(DrawableElement el1) {
        ...
        if (el1.getPgenCategory().equals("Lines")
                || el1.getPgenCategory().equals("Front")
    ...
    /**
     * Performs the interpolation between the two selected elements using the
     * properties(parameters) specified in the Interpolation Dialog. New
     * elements are created and are added to the Pgen Resource. This method is
     * invoked when the Interpolation button on the dialog is pressed.
     */
    public void performInterpolation() {

        /*
         * get interpolation properties from the dialog reverse the interval is
         * starting time > end time.
         */
        InterpolationProperties props = new InterpolationProperties(
                interpDlg.getStartTime(), interpDlg.getEndTime(),
                interpDlg.getInterval());

        /*
         * perform the interpolation and get back new elements
         */
        List<AbstractDrawableComponent> deList = PgenInterpolator.interpolate(
                selectedEls.get(0).getPrimaryDE(),
                selectedEls.get(1).getPrimaryDE(), props,
                getDescriptor(mapEditor));

        /*
         * Add any new elements to the Pgen Resource
         */
        if (deList.size() > 0) {
            drawingLayers.addElements(deList);
        }
