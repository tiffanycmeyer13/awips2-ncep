...
public class VolcanoCreateDlg extends AttrDlg implements ISinglePoint {
    ...
    /**
     * Volcano creating with Layers button NOT used but keeping here for
     * reference
     */
    Button btnLay = null;
    ...
    /**
     * flag for the Volcano info sources from file or from user manual input
     */
    ...
    /**
     * method overridden of super class to create dialog area for this class
     */
    @Override
    public Control createDialogArea(Composite parent) {
        ...
        new TextFieldListener(
                ...
                new Control[] { btnVol }// 2010-03-17: taking off btnLay using
                                        // Product Center
        );
    ...
    private void createArea1(Group top2) {
        ...
        for (int i = 0; i < SigmetInfo.VOL_NAME_BUCKET_ARRAY.length; i++) {
            if (i == 0) {// first option is entering name
                ...
                java.util.List<String> list = SigmetInfo.VOLCANO_BUCKET_MAP
    ...
    private void createArea4(Group top) {
        ...
        btnVol.addListener(SWT.Selection, new Listener() {
            @Override
            public void handleEvent(Event e) {
                if (txtVol != null && txtVol.getText() != null
                        && txtVol.getText().length() > 0) {
                    createVol();
                    cancelPressed();
                }
            }
    ...
    private void createArea5(Group top) {
        ...
        btnGo.addListener(SWT.Selection, new Listener() {
            @Override
            public void handleEvent(Event e) {
                VolcanoVaaAttrDlg vaDlg = VolcanoVaaAttrDlg
                        .getInstance(VolcanoCreateDlg.this.getParentShell());
                ...
                VaaInfo.setVolcanoFields(vol, comboType.getText().trim(),
                        false);
                drawingLayers.addElement(vol);// 2010-03-17//VaaInfo.setVolcano(vol);
                VaaInfo.VOL_PROD_MAP.put(vol, drawingLayers.getActiveProduct());
     ...
     *
     * @param boolean:
     *            flag indicating enable/disable
     */
    ...
    private void setTextToTxts(String volName) {
        ...
        txtEle.setText(VaaInfo.getFootTxtFromMeter(stn.getElevation(), 1));
    ...
    private String getLocText(Station stn) {
        String s = PgenUtil.getLatLonStringPrepend(
                new Coordinate[] {
    ...
    public void createVol() {
        ...
        elem = (Volcano) def.create(DrawableType.VAA, this, pgenCategory,
                pgenType, new Coordinate(stn.getLongitude(), stn.getLatitude()),
        ...
        if (elem != null) {
            ...
            VaaInfo.VOL_PROD_MAP.put(elem, drawingLayers.getActiveProduct());
            mapEditor.refresh();
        }
    }
    ...
    private void setStation(boolean flag) {
        if (flag) {
            ...
            stn.setElevation(
                    VaaInfo.getMeterIntFromFoot(txtEle.getText().trim()));
    ...
    /**
     * class with Text fields listeners for checking the validity of input
     * values
     *
     */
    class TextFieldListener extends KeyAdapter
        ...
        /**
         * checks the Text fields 1. null or empty 2. invalid inputs of Lat/Lon
         */
        ...
        public void modifyText(ModifyEvent e) {
            ...
            if (isManualInput) {
                ...
                /*
                 * some Lat/Lon from file are NOT standard: Example: Volcano
                 * Salak in java: S671E10673 No check performed on volcanos from
                 * the file
                 */
                for (Control cont : controls) {
                    cont.setEnabled(true);
