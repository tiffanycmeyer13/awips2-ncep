...
import static gov.noaa.nws.ncep.ui.pgen.gfa.GfaInfo.DESK_XPATH;
import static gov.noaa.nws.ncep.ui.pgen.gfa.GfaInfo.FCSTHR_XPATH;
import static gov.noaa.nws.ncep.ui.pgen.gfa.GfaInfo.HAZARD_XPATH;
import static gov.noaa.nws.ncep.ui.pgen.gfa.GfaInfo.ISSUE_TYPE_XPATH;
import static gov.noaa.nws.ncep.ui.pgen.gfa.GfaInfo.getDefaultRGB;
import static gov.noaa.nws.ncep.ui.pgen.gfa.GfaInfo.selectNodes;
 ...
 * 05/11         #?           J. Wu        Retrieve cycle hour from
 *                                         PgenCycleTool
 ...
 * 02/12         597          S. Gurung    Moved snap functionalities to
 *                                         SnapUtil from SigmetInfo.
 * 05/12         637          Archana.S    Updated the code to update the filter
 *                                         hr in PgenFilterDlg based on the
 *                                         current Fcst hr selected.
 * 05/12         808          J. Wu        update vor text.
 * 07/12         663          Q. Zhou      Add selected Gfa in movetext
 *                                         listener. Added get/set for
 *                                         MoveTextBtn
 * 12/12         908          B. Yin       Added empty items in drop-down menus
 *                                         for multi-selecting
 * 12/12         937          J. Wu        Update G_Airmet layers/hazard - "C&V"
 * 05/13         610          J. Wu        Implemented FZLVL range (TTR425)
 * 06/13         #674/TTR426  J. Wu        Added color settings for surface
 *                                         FZLVLs.
...
public class GfaAttrDlg extends LineAttrDlg implements IGfa {
    ...
    private final String[] LABELS = { "Hazard:", "Fcst Hr:", "Tag:", "Desk:",
            "Issue Type:", "", "" };
    ...
    private final LinkedHashMap<String, Boolean> typeCheckboxes = new LinkedHashMap<>();

    private String addRemoveBtnLabel = "";

    private final LinkedHashMap<String, Boolean> addRemoveDlgCheckboxes = new LinkedHashMap<>();
    ...
    private final ArrayList<String> popupCheckboxes = new ArrayList<>();

    private final HashMap<String, Boolean> requiredFields = new HashMap<>();

    private final HashMap<String, Label> requiredCrosses = new HashMap<>();

    private Label warning;

    private final String RED_CROSS_PATH = PgenStaticDataProvider.getProvider()
            .getFileAbsolutePath(PgenStaticDataProvider.getProvider()
                    .getPgenLocalizationRoot() + PGEN_RED_CROSS_IMG);
    ...
    protected void initializeComponents() {
        ...
        comboGroup.setLayoutData(
                new GridData(SWT.LEFT, SWT.BEGINNING, false, true));

        hazardSpecificGroup = new Group(top, SWT.BORDER);
        hazardSpecificGroup.setLayoutData(
                new GridData(SWT.LEFT, SWT.BEGINNING, false, true));
    ...
    private Group createPanelCombo() {
        ...
        hazardCbo = createCombo(panelComboGroup, HAZARD_XPATH,
                hazardIndexLastUsed);
        fcstHrCbo = createFcstHrCombo(panelComboGroup, FCSTHR_XPATH,
                fcstHrIndexLastUsed);
        ...
        issueTypeCbo = createCombo(panelComboGroup, ISSUE_TYPE_XPATH,
                issueTypeIndexLastUsed);
        ...
        textVOR = createTextAttr(panelComboGroup, TEXT_WIDTH, TEXT_HEIGHT, 6,
                true, false);
    ...
    private void createColorButtonSelector() {
        ...
        if (rgbLastUsed == null) {
            ...
            RGB rgb = getDefaultRGB(hazardCbo.getItem(hazardIndexLastUsed),
                    getGfaFcstHr());

            if (hazardCbo.getItem(hazardIndexLastUsed)
                    .equalsIgnoreCase("FZLVL")) {
        ...
        if (hazardCbo.getText().equalsIgnoreCase("FZLVL")) {
            ...
            if (levelCmb != null && !levelCmb.isDisposed()) {
                ...
                if (lvl.equalsIgnoreCase("SFC")) {
                    RGB useFzlvlSfcClr = getDefaultFzlvlSfcColor(
                            getGfaFcstHr());
    ...
    private void createBottomPanel() {
        ...
        if (fcstHrCbo.getText().indexOf('Z') > -1
                || ("Other".equalsIgnoreCase(fcstHrCbo.getText())
                        && !otherText.getText().contains("-"))) {
        ...
        GridData gridData = new GridData(GridData.BEGINNING, SWT.NONE, true,
                false, 2, 1);
        ...
        areaText = createTextAttr(bottomGroup, 100, TEXT_HEIGHT, 1, false,
                true);

        lbl = new Label(bottomGroup, SWT.RIGHT);
        lbl.setText("  Beginning:");
        lbl.setLayoutData(new GridData(SWT.RIGHT, SWT.BEGINNING, true, true));
        beginningText = createTextAttr(bottomGroup, 200, TEXT_HEIGHT, 1, true,
                true);

        lbl = new Label(bottomGroup, SWT.RIGHT);
        lbl.setText("  Ending:");
        lbl.setLayoutData(new GridData(SWT.RIGHT, SWT.BEGINNING, true, true));
        endingText = createTextAttr(bottomGroup, 200, TEXT_HEIGHT, 1, true,
                true);
        ...
        statesText = createTextAttr(bottomGroup, 300, TEXT_HEIGHT, 5, true,
                true);
    }

    /**
     * Adds a checkbox to hazardSpecificGroup with the attributes specified in
     * gfa.xml
     *
     * @param node
     */
    ...
    private void processPopupNode(final Group group, Node node) {
        ...
        btn.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                try {
                    if (addRemoveTypeDlg == null
                            || addRemoveTypeDlg.getShell() == null
                            || addRemoveTypeDlg.getShell().isDisposed()
                            || addRemoveTypeDlg.getShell()
                                    .getParent() == null) {
                        addRemoveTypeDlg = new AddRemoveTypeDlg(
                                GfaAttrDlg.this.getShell());
                        addRemoveTypeDlg.parentLabel = addRemoveBtnLabel;
                    }
                    openAttrDlg(addRemoveTypeDlg);
                    Point p = addRemoveTypeDlg.getShell().getLocation();
                    addRemoveTypeDlg.getShell()
                            .setLocation(p.x + TEXT_WIDTH + 160, p.y + 130);
    ...
    private Text processTextNode(final Group group, Node node) {
        ...
        boolean scrollable = "true"
                .equalsIgnoreCase(node.valueOf("@scrollable"));
        // true by default
        boolean editable = !"false".equalsIgnoreCase(node.valueOf("@editable"));
        ...
        boolean digitsOnly = "digitsOnly"
                .equalsIgnoreCase(node.valueOf("@characterType"));
        if (digitsOnly || "digitsAndSlash"
                .equalsIgnoreCase(node.valueOf("@characterType"))) {

            final boolean slashAllowed = "digitsAndSlash"
                    .equalsIgnoreCase(node.valueOf("@characterType"));
            final String padWithZeros = node.valueOf("@padWithZeros");

            txt.addVerifyListener(
                    new VerifyListenerDigitsOnly(slashAllowed, CHAR_LIMIT));

            FocusListenerPadding fl = new FocusListenerPadding(padWithZeros,
                    digitsOnly);
            if ("Bottom".equalsIgnoreCase(lblStr)) {
                fl.maxFirstToken = 999; // otherwise 450 for "Top"
            }
    ...
    private void processFzlNode(final Group group, Node node) {
        ...
        txt1.addModifyListener(new ModifyListener() {
            ...
            public void modifyText(ModifyEvent e) {
                ...
                lbl2.setForeground(
                        Display.getDefault().getSystemColor(colorInt));
            }
        });

        // update txt2, then txt1, otherwise txt2 will be blanked
        if (values.get(Gfa.TOP_BOTTOM) != null) {
            txt2.setText(nvl(values.get(Gfa.FZL_TOP_BOTTOM)));
        }
        if (values.get(Gfa.TOP_BOTTOM) != null) {
            txt1.setText(nvl(values.get(Gfa.TOP_BOTTOM)));
        }
    ...
    public void populateTagCbo() {
        ...
        tagCbo.removeAll();
        TreeSet<String> tags = new TreeSet<>(new Comparator<String>() {
            @Override
            public int compare(String s1, String s2) {
                if (s1.equals(s2)) {
                    return 0;
                } else if ("New".equals(s1)) {
                    return -1;
                } else if ("New".equals(s2)) {
                    return 1;
                }
        ...
        if (drawingLayers != null) {
            ...
            List<AbstractDrawableComponent> selected = drawingLayers
                    .getAllSelected();
            for (AbstractDrawableComponent adc : all) {
                if (!(adc instanceof Gfa) || !((Gfa) adc).isSnapshot()
                        || !((Gfa) adc).getGfaHazard()
                                .equals(this.getGfaHazard())
                        || !((Gfa) adc).getGfaDesk()
                                .equals(this.getGfaDesk())) {
                    continue;
                }
                Gfa gfa = (Gfa) adc;
                tags.add(gfa.getGfaTag());
            }
            ArrayList<String> reinsertWithStar = new ArrayList<>();
            for (String tag : tags) {
                if (needStar(tag)) {
                    reinsertWithStar.add(tag);
                }
            }
        ...
        if (drawingLayers != null) {
            ...
            for (AbstractDrawableComponent adc : all) {
                if (adc instanceof Gfa && ((Gfa) adc).isSnapshot()
                        && ((Gfa) adc).getGfaHazard()
                                .equals(this.getGfaHazard())
                        && ((Gfa) adc).getGfaFcstHr()
                                .equals(this.getGfaFcstHr())
     ...
     * @return
     */
    private Combo createFcstHrCombo(Group group, String xPath,
            int indexToSelect) {
    ...
    private void addSelectionListeners() {
        ...
        SelectionAdapter saveSettings = new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                if (!PgenSession.getInstance().getPgenPalette()
                        .getCurrentAction().equalsIgnoreCase("MultiSelect")) {
                    populateTagCbo();
                }
            }
        };
    ...
    /**
     * Enables the filter hour corresponding to the selected gfaFcsthr if the
     * PgenFilter dialog is open.
     *
     * @param gfaFcstHr
     *            - the selected fcst hour in the GFA dialog
     */
    private void selectFilterHourWhenGfaFcstHourIsChanged(String gfaFcstHr) {
        for (Button button : PgenFilterDlg.hourBtns) {
            if (!button.getSelection()) {
                String buttonText = button.getText();
                String filterHr = (buttonText.endsWith("+")
                        ? buttonText.replace('+', ' ').trim() : buttonText);
     ...
     * @param height
     */
    private Text createTextAttr(Group group, int width, int height,
            int horizontalSpan, boolean scrollable, boolean editable) {
    ...
    public void setAttrForDlg(IAttribute iattr) {
        if (iattr instanceof IGfa) {
            ...
            if (hazardCbo != null) {
                ...
                if (index == -1 && attr.getGfaHazard() != null
                        && !attr.getGfaHazard().contains("-")) {
                    index = 1;
                }
            ...
            if (fcstHrCbo != null) {
                ...
                // no dash, it is a snapshot
                if (nvl(str).indexOf("-") == -1 && !nvl(str).isEmpty()) {
                    try {
                        int i = Integer.parseInt(str); // snapshot hour
                        int h = this.getGfaCycleHour(); // cycle hour

                        h = (h + i) % 24; // to take care of the next day
                                          // situation
                ...
                if (index == -1 && !nvl(str).isEmpty() && otherText != null
            ...
            if (type != null) {
                typeLastUsed = (attr.getGfaType() == null) ? ""
                        : attr.getGfaType();
                type.setText(typeLastUsed);

                /*
                 * parse type and update typeCheckboxes and
                 * addRemoveDlgCheckboxes Note - The "/" to separate CIG and VIS
                 * is temporarily replaced as ":" since types for VIS is also
                 * separated by "/".
                 */
                String[] types = typeLastUsed.replace("/VIS", ":VIS")
                        .split(":");
    ...
    /**
     * Updates the type textfield with the information based on the checked type
     * buttons in this dialog and in addRemoveTypeDlg.
     */
    private void updateType() {
        ...
        for (String key : addRemoveDlgCheckboxes.keySet()) {
            if (addRemoveDlgCheckboxes.get(key)) {
                if (sb2.length() == 0) {
                    sb2.append(addRemoveBtnLabel + " ");
                }
                sb2.append(key).append("/");
            }
        }
        if (sb2.length() > 0) {
            // non empty sb2 always ends with "/"
        ...
        if (typeStr.contains("CIG")) {
            values.put("CIG", "BLW_010");
        }
        if (typeStr.contains("VIS")) {
            values.put("VIS", "BLW_3SM");
        }
        ...
        for (String key : widgets.keySet()) {
            ...
            if (w instanceof Text) {
                ...
                if (Gfa.TOP_BOTTOM.equalsIgnoreCase(key)) {
                    ...
                    values.put(Gfa.TOP,
                            (splitValue[0].isEmpty()) ? null : splitValue[0]);
                    values.put(Gfa.BOTTOM,
                            (splitValue.length > 1) ? splitValue[1] : null);
                }
                values.put(key, v);
                ...
                if (key.equalsIgnoreCase(Gfa.LEVEL)) {
                    ...
                    RGB clr = getDefaultRGB(
                            hazardCbo.getItem(hazardIndexLastUsed),
                            getGfaFcstHr());
                    if (hazardCbo.getText().equalsIgnoreCase("FZLVL")) {
                        if (lvl.equalsIgnoreCase("SFC")) {
    ...
    public String getGfaFcstHr() {
        if (fcstHrCbo != null && !fcstHrCbo.isDisposed()) {
            ...
            if (str.indexOf("Z") > -1) {
                // replace all the terms containing "Z" with "Z", removing
                // everything else
    ...
    /**
     * Returns a copy of a map with all the values from hazard dependent gui
     * panel.
     *
     * @return
     */
    ...
    public void redrawHazardSpecificPanel() {
        ...
        // update color
        rgbLastUsed = getDefaultRGB(hazardCbo.getItem(hazardIndexLastUsed),
                getGfaFcstHr());
        cs.setColorValue(rgbLastUsed);
    ...
    private void createOtherText() {
        ...
        if ("Other".equalsIgnoreCase(fcstHrCbo.getText())) {
            ...
            if (otherTextLastUsed != null) {
                otherText.setText(otherTextLastUsed);
            }
            ...
            textVOR.dispose();
            cs.dispose();
            textVOR = createTextAttr(panelComboGroup, TEXT_WIDTH, TEXT_HEIGHT,
                    6, true, false);
            createColorButtonSelector();
    ...
    public void okPressed() {
        ...
        if (fcstHrCbo != null && !fcstHrCbo.isDisposed()
                && "Other".equalsIgnoreCase(fcstHrCbo.getText())
                && otherText != null && !otherText.isDisposed()) {
    ...
    public int open() {
        ...
        switchHazard(PgenSession.getInstance().getCurrentResource()
    ...
    private String padTime(String in) {
        if (in.matches("[01234569]|12")) {
            return in += ":00";
        }
        if (in.matches("12:|[012345]:")) {
            return in += "00";
        }
        if (!in.matches(TIME)) {
            return in;
    ...
    public boolean validateRequiredFields() {
        ...
        if (new Boolean(true).equals(requiredFields.get("type")) && type != null
                && !type.isDisposed() && type.getText().isEmpty()) {
            displayCrosses(true);
            return false;
        }
    ...
    private void updateColorWhenOtherTextChanged(String s) {
        ...
        if ("Other".equals(fcstHrCbo.getText())) {
            ...
            RGB clr = getDefaultRGB(hazardCbo.getItem(hazardIndexLastUsed),
                    getGfaFcstHr());
            cs.setColorValue(clr);
        }
    }
    ...
    private class VerifyListenerDigitsOnly implements VerifyListener {
        private final boolean slashAllowed;
        ...
        public void verifyText(VerifyEvent e) {
            ...
            String str = ((Text) e.widget).getText().substring(0, e.start)
                    + e.text;
            ...
            if (slashAllowed) {
                // from 0 to 3 digits (\\d{0,3}) or (|) from 1 to 3 digits +
                // slash (/) + up to 3 digits again (\\d{0,3})
                e.doit = str.matches(
                        "\\d{0,3}|\\d{1,3}/|\\d{1,3}/\\d{0,3}|\\d{1,3}/S|\\d{1,3}/SF|\\d{1,3}/SFC|\\d{1,3}/F|\\d{1,3}/FZ|\\d{1,3}/FZL");
            } else {
        ...
        public void focusLost(FocusEvent e) {
            ...
            if (!digitsToPad.isEmpty()) {
                ...
                for (int i = 0; i < split.length; i++) {
                    if (!digitsOnly && i == 1
                            && split[i].toUpperCase().startsWith(
                                    "SFC".substring(0, split[i].length()))) {
                        split[i] = "SFC";
                    } else if (!digitsOnly && i == 1
                            && split[i].toUpperCase().startsWith(
                                    "FZL".substring(0, split[i].length()))) {
                        split[i] = "FZL";
                    } else {
                        split[i] = padWithZeros(split[i], padInt);
                    }
                    if (i == 0 && Integer.parseInt(split[i]) > maxFirstToken) {
                        split[i] = "" + maxFirstToken; // for "Top" fields
                    }
                }
            }
            String out = "";
    ...
    public void setEnableStatesButton(boolean enabled) {
        if (statesBtn == null || statesBtn.isDisposed()) {
            return;
        }
        if (enabled) {
            statesBtn.setBackground(
    ...
    private class AddRemoveTypeDlg extends AttrDlg {
        ...
        public void createButtonsForButtonBar(Composite parent) {
            ...
            getButton(IDialogConstants.CLOSE_ID)
        ...
        public void handleShellCloseEvent() {
            // do not need to go to SelectingMode, so overriding the AttrDlg's
            // method.
        ...
        public Control createDialogArea(Composite parent) {
            ...
            for (final String labelStr : popupCheckboxes) {
                ...
                btn.addSelectionListener(new SelectionAdapter() {
                    @Override
                    public void widgetSelected(SelectionEvent e) {
                        addRemoveDlgCheckboxes.put(labelStr,
                                btn.getSelection());
                        updateType();
                    }
                });
    ...
    /*
     * Update the active layer in the product manage GUI or the layering control
     * window to the current Gfa hazard type.
     */
    private void linkHazardWithLayer(String layer) {
        ProductManageDialog productManageDlg = PgenSession.getInstance()
                .getCurrentResource().getProductManageDlg();
        PgenLayeringControlDialog layeringControlDlg = PgenSession.getInstance()
                .getCurrentResource().getLayeringControlDlg();
        if (productManageDlg != null && productManageDlg.isOpen()) {
            productManageDlg.switchLayer(layer);
        } else if (layeringControlDlg != null && layeringControlDlg.isOpen()) {
            layeringControlDlg.switchLayer(layer);
        }
    }
     ...
     *
     * This used when the first time a smear/outlook is selected so the bottom
     * panel can be generated.
     *
