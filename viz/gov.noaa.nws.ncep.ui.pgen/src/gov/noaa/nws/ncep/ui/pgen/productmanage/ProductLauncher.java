...
/**
 *
 * Contribution items added to the PGEN product "Launch" drop-down menu which
 * display the product types available for launching a product template. The
 * product types are configured and saved in "productTypes.xml".
 *
 * <pre>
 *
 * SOFTWARE HISTORY
...
public class ProductLauncher extends ContributionItem {
    ...
    private void quickLaunch(String prdtype) {
         ...
         * product type. If no layers specified for the type, a default layer is
        ...
        if (players != null) {
            for (PgenLayer plyr : players) {
                ...
                Color clr = new Color(plyr.getColor().getRed(),
                        plyr.getColor().getGreen(), plyr.getColor().getBlue(),
