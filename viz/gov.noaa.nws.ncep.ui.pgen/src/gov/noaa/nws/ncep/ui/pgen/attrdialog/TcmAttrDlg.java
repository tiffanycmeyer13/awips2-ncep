...
public class TcmAttrDlg extends AttrDlg implements ITcm, SelectionListener {
    ...
    private static final String[] fcstHrStr = { "OBS", "06", "12", "24", "36",
            "48", "72", "96", "120", "144" };
    ...
    private final TcmWindQuarters waves = null;
    ...
    /*
     * read in all possible selections for the pulldown menus from the
     * tcainfo.xml file
     */
    private static void readOptions() {
        ...
        File tcainfoFile = PgenStaticDataProvider.getProvider().getStaticFile(
                PgenStaticDataProvider.getProvider().getPgenLocalizationRoot()
                        + TcaAttrDlg.PGEN_TCA_ATTR_INFO);

        try {
            info = (TcaAttrInfo) SerializationUtil
                    .jaxbUnmarshalFromXmlFile(tcainfoFile.getAbsoluteFile());
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public Control createDialogArea(Composite parent) {
        ...
        /*
         * Initialize section that allows users to add/modify breakpoints for
         * each advisory
         */
    ...
    private void createStormInfoArea(Group g1) {
        ...
        validTime.addVerifyListener(new VerifyListener() {
            ...
            public void verifyText(VerifyEvent ve) {
                ...
                if (Character.isDigit(ve.character) || ve.character == BACKSPACE
                        || ve.character == DELETE) {
                    ve.doit = true;
                } else {
                    ve.doit = false;
    ...
    private void createTcmFcst(Group g2) {
        ...
        /*
         * Composite used to group Apply, delete segment, and new segment
         * buttons
         */
    ...
    public void setAttrForDlg(IAttribute attr) {
        ...
        if (attr instanceof ITcm) {
            ...
            this.setCentralPressure(itcm.getCentralPressure());
    ...
    public Calendar getAdvisoryTime() {
        ...
        advTime.set(validDate.getYear(), validDate.getMonth(),
        ...
        if (e.widget instanceof Button) {
            ...
            if (b.getText().equals(ADD)) {
                ...
                if (tcm == null) {
                    tcm = new Tcm(this.getStormType(), this.getStormNumber(),
                            ...
                            this.getBasin(), this.getEyeSize(),
                            this.getPositionAccuracy(), this.isCorrection(),
                            this.getAdvisoryTime(), this.getCentralPressure());
                    drawingLayers.addElement(tcm);
                } else {
                    Tcm newTcm = (Tcm) tcm.copy();
                    drawingLayers.replaceElement(tcm, newTcm);
                    tcm = newTcm;
                }

                TcmFcst elem = (TcmFcst) new DrawableElementFactory().create(
                        DrawableType.TCM_FCST, this, pgenCategory, pgenType,
                        new Coordinate(getLon(), getLat()),
             ...
             */
                ...
                for (TcmFcst fcst : newTcm.getTcmFcst()) {
                    ...
                    if (this.getFcstHr() == fcst.getFcstHr()) {
                        newFcst = (TcmFcst) new DrawableElementFactory().create(
                                DrawableType.TCM_FCST, this, pgenCategory,
                                pgenType, new Coordinate(getLon(), getLat()),
                                drawingLayers.getActiveLayer());
                ...
                if (newFcst != null) {
                    ...
                    if (ii == 0) {
                        newTcm.getWaveQuarters()
                                .setLocation(newFcst.getLocation());
                    }
                }

                drawingLayers.replaceElement(tcm, newTcm);
                drawingLayers.removeGhostLine();
                drawingLayers.setSelected(newTcm);
                this.tcm = newTcm;
                mapEditor.refresh();
    ...
    /**
     * Gets forecast hour selection from TCM dialog. OBS = 0
     */
    @Override
    ...
    public TcmWindQuarters getWaveQuarters() {
        if (waves == null) {
            return new TcmWindQuarters(tcm.getTcmFcst().get(0).getLocation(), 0,
                    getRadius(ne12ftField), getRadius(se12ftField),
                    getRadius(sw12ftField), getRadius(nw12ftField));
        } else {
            return waves;
        }
    }
    ...
    private void setTcmFcstInfo(TcmFcst fcst) {
        ...
        ne34Field.setText(
                Integer.toString((int) fcst.getQuarters()[0].getQuarters()[0]));
        nw34Field.setText(
                Integer.toString((int) fcst.getQuarters()[0].getQuarters()[3]));
        sw34Field.setText(
                Integer.toString((int) fcst.getQuarters()[0].getQuarters()[2]));
        se34Field.setText(
                Integer.toString((int) fcst.getQuarters()[0].getQuarters()[1]));

        ne50Field.setText(
                Integer.toString((int) fcst.getQuarters()[1].getQuarters()[0]));
        nw50Field.setText(
                Integer.toString((int) fcst.getQuarters()[1].getQuarters()[3]));
        sw50Field.setText(
                Integer.toString((int) fcst.getQuarters()[1].getQuarters()[2]));
        se50Field.setText(
                Integer.toString((int) fcst.getQuarters()[1].getQuarters()[1]));

        ne64Field.setText(
                Integer.toString((int) fcst.getQuarters()[2].getQuarters()[0]));
        nw64Field.setText(
                Integer.toString((int) fcst.getQuarters()[2].getQuarters()[3]));
        sw64Field.setText(
                Integer.toString((int) fcst.getQuarters()[2].getQuarters()[2]));
        se64Field.setText(
                Integer.toString((int) fcst.getQuarters()[2].getQuarters()[1]));
    }
