 ...
 * 10/10         289      Archana      Added overridden method handleKeyDown()
 *                                     to PgenSelectHandler to accept the delete
 *                                     key. Updated the method handleMouseDown()
 ...
 * Feb 10, 2011           Chin Chen    fixed null pointer exception issue
 * 02/11         ?        B. Yin       Automatically add/del ]/[ for front
 *                                     labels
 * 04/11         ?        B. Yin       Re-factor IAttribute
 * 02/12         TTR456   Q.Zhou       Added parameters to setTrack()
 * 02/12         597      S. Gurung    Removed snapping for CONV_SIGMET and
 *                                     NCON_SIGMET. Moved snap functionalities
 *                                     to SnapUtil from SigmetInfo.
 * 02/12         TTR 525  B. Yin       Mkae sure points don't move when
 *                                     selecting.
 * 04/12         750      Q. Zhou      Modified handleMouseDownMove for loc
 *                                     null. Added tempLoc and inOut to trace
 *                                     position of single position element
 ...
 * 06/12         777      Q. Zhou      Added isNewTrack flag on Track to get
 *                                     first time differently
 * 07/12         663      Q. Zhou      Modified handleMouseDown for preselected
 *                                     DE.
 * 08/12         803      Q. Zhou      Fixed Front text of 2 lines. Modified
 *                                     handleMouseDownMove.
 * 11/12         911      J. Wu        TTR 652 - prevent invalid GFA polygon
 *                                     when dragging GFA points.
 * 03/13         927      B. Yin       Moved out the handler class.
 * 11/13         1063     B. Yin       Modified getMouseHandler to handle
 *                                     non-PgenSelectingMouseHandler
 * 11/13         1081     B. Yin       Removed prevDe and added getSelectedDE
 *                                     method.
 ...
 *                                     clicks.
 * Nov 30, 2016  17954    bugenhagen   Check for null palette in deactivateTool.
...
public class PgenSelectingTool extends AbstractPgenDrawingTool
    ...
    public IInputHandler getMouseHandler() {
        ...
        // e.g. deleting barb when a jet is selected (TTR 902)
        if (this.mouseHandler == null
                || !(this.mouseHandler instanceof PgenSelectHandler)
                || this.mapEditor != ((PgenSelectHandler) mouseHandler)
                        .getMapEditor()
                || !this.drawingLayers.matches(
                        ((PgenSelectHandler) mouseHandler).getPgenrsc())) {
            this.mouseHandler = new PgenSelectHandler(this, mapEditor,
    ...
    public void applyBarbAttrOnJet(IAttribute attr, String type) {
        ...
        if (jet != null) {
            ...
            drawingLayers.setSelected(newJet);
            jet = newJet;
    ...
    public void changeSelectedLineType(String type) {
        ...
        if (currentDe != null && (currentDe.getPgenCategory()
                .equalsIgnoreCase("Lines")
                || currentDe.getPgenCategory().equalsIgnoreCase("Front"))) {

            AttrDlg dlg = AttrDlgFactory.createAttrDlg(
                    currentDe.getPgenCategory(), type, PlatformUI.getWorkbench()
            ...
            drawingLayers.replaceElement(de, ln);
    ...
    public DrawableElement getSelectedDE() {
        ...
        if (drawingLayers != null) {
            de = drawingLayers.getSelectedDE();
        }
