...
public class PgenWatchBoxAddDelCntyHandler extends InputHandlerDefaultImpl {
    ...
    private final PgenWatchBoxModifyTool wbTool;
     ...
     *            box
     */
    public PgenWatchBoxAddDelCntyHandler(AbstractEditor mapEditor,
            PgenResourceList drawingLayers, WatchBox wb,
            PgenWatchBoxModifyTool tool) {
    ...
    public boolean handleMouseDown(int anX, int aY, int button) {
        ...
        if (button == 1) {
            ...
            // apply filter
            FeatureCollection<SimpleFeatureType, SimpleFeature> fc = counties
                    .subCollection(createFilter(click));
            ...
            if (ugc != null) {
                ...
                List<SPCCounty> cntyTbl = PgenStaticDataProvider.getProvider()
                        .getSPCCounties();
            ...
            if (gotCnty) {
                ...
                if (inList) {
                    if (((WatchBoxAttrDlg) wbTool.attrDlg).getWatchInfoDlg()
                            .isClusteringOn()) {
                ...
                drawingLayers.replaceElement(wb, newWb);
                drawingLayers.setSelected(newWb);

                wb = newWb;

                ((WatchBoxAttrDlg) wbTool.attrDlg).setWatchBox(newWb);
                ((WatchBoxAttrDlg) wbTool.attrDlg).getWatchInfoDlg()
                        .clearCwaPane();
                ((WatchBoxAttrDlg) wbTool.attrDlg).getWatchInfoDlg()
        ...
        for (SPCCounty cnty : cntyTbl) {
            if (cnty.getShape() != null && cnty.getName() != null) {
                ...
                if (countyGeo != null) {
                    ...
                    SimpleFeature feature = fbuild
                            .buildFeature(cnty.getUgcId());
    ...
    private Filter createFilter(Point loc) {
        ...
        Filter filter = ff.contains(ff.property("Location"), ff.literal(loc));
