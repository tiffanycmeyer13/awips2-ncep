...
public class CloudAttrDlg extends AttrDlg implements ILine {
    ...
    /**
     * Creates a cloud attribute dialog if the dialog does not exist and returns
            ...
            public void widgetSelected(SelectionEvent e) {
                if (((Button) e.widget).getSelection()) {
                    if (cloudTool != null && cloudTool.getLabeledLine() != null
                            && cloudTool.getLabeledLine().getLines()
                                    .size() > 0) {
                        ...
                        flipBtn.setSelection(false);
                        try {
                            labelDlg = new LabelAttrDlg(
                                    CloudAttrDlg.this.getParentShell());
                            labelDlg.setBlockOnOpen(false);
                            labelDlg.open();
                            labelDlg.setPgenType("MID_LEVEL_CLOUD");
                            labelDlg.setDefaultAttr();
                        } catch (VizException e1) {
                            statusHandler.error("Error from LabelAttrDlg", e1);
                        }
            ...
            public void widgetSelected(SelectionEvent e) {
                if (cloudTool != null && cloudTool.getLabeledLine() != null
                    ...
                    try {
                        ...
                        labelDlg.setDrawingLayers(drawingLayers);
                    } catch (VizException e1) {
                        statusHandler.error("Error from LabelAttrDlg", e1);
                    }
                }
    ...
    public void setAttr(AbstractDrawableComponent adc) {
        if (adc instanceof Cloud) {
            setAttrForDlg(((Cloud) adc).getLines().get(0));
        }
    }
    ...
    public void okPressed() {
        if (cloudTool != null && cloudTool.getLabeledLine() != null) {
            ...
            Iterator<AbstractDrawableComponent> it = newll
    ...
    private class LabelAttrDlg extends MidLevelCloudAttrDlg {
        ...
        public void okPressed() {
            ...
            // apply to all labels
            for (gov.noaa.nws.ncep.ui.pgen.elements.labeledlines.Label lbl : newCloud
                    .getLabels()) {
            ...
            if (!newCloud.getLabels().isEmpty()) {
                AttrSettings.getInstance()
                        .setSettings(newCloud.getLabels().get(0).getSpe());
            }

            // put the updated cloud in PGEN resource and refresh
            CloudAttrDlg.this.drawingLayers
                    .replaceElement(cloudTool.getLabeledLine(), newCloud);
            drawingLayers.setSelected(newCloud);
