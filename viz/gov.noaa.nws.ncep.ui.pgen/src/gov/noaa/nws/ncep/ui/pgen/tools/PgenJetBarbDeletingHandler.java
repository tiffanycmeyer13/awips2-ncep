...
public class PgenJetBarbDeletingHandler extends InputHandlerDefaultImpl {
     ...
     * @param jet
     */
    public PgenJetBarbDeletingHandler(AbstractEditor mapEditor,
        ...
        if (button == 1) {
            if (barbSelected != null) {
                ...
                newJet.remove(newJet.getNearestComponent(
                        ((ISinglePoint) (barbSelected.getPrimaryDE()))
                                .getLocation()));
                ...
                // de-select the barb
                drawingLayers.removeSelected(barbSelected);
                ...
                if (!barbSelected.getName().equalsIgnoreCase("WindInfo")) {
                    ...
                    drawingLayers.addSelected(barbSelected);

                    // put red dots
                    DECollection dec = new DECollection();
                    for (Coordinate pt : barbSelected.getPoints()) {
                        Symbol redDot = new Symbol(null,
                                new Color[] { Color.red }, 2.5f, 7.5, false, pt,
                                "Marker", "DOT");
                        dec.add(redDot);
                    }
                    drawingLayers.setGhostLine(dec);
                }
            }
