 ...
 * 12/11         522      B. Yin     Made the linkage between the filter dialog
 *                                   and the GFA dialog.
 * 12/11                  Q.Zhou     Read HOURS from filterHour.xml instead of
 *                                   fixed string.
 * 05/12         637      Archana.S  Updated the code to update the fcst hr in
 *                                   GfaAttrDialog based on the current filter
 *                                   hour selected. Updated the hot-key behavior
 *                                   to match legacy.
 * 11/12         912      B. Yin     When an hour is turned off, de-select and
...
public class PgenFilterDlg extends CaveJFACEDialog {
    ...
    // Pgen resource
    private PgenResourceList drawingLayers;
    ...
    // filter map
    private final HashMap<String, ElementFilter> filterMap;

    protected FilterButtonKeyListener filterBtnKeyLstnr;

    // filter to block all hours
    private final ForecastHourFilter blockAll;
    ...
    private class FilterButtonKeyListener implements KeyListener {
        ...
        List<Integer> enabledButtonIndexList = new ArrayList<>(0);
        ...
        public void keyPressed(KeyEvent e) {
            ...
            for (index = 0; index < hoursArrayLength; index++) {
                ...
                if (hourBtns[index].getSelection()) {
                    ...
                    /*
                     * increment a counter only if it does not exceed the total
                     * number of buttons
                     */
            ...
            /*
             * this is to match legacy behavior. If all the buttons are enabled,
             * then we disable all of them and enable only the right-most or
             * left-most filter button based on the hot key pressed
             */
            ...
            case '[':
                ...
                /*
                 * If all the buttons are enabled, pressing the '[' key should
                 * deactivate all filters except the first one. If none of the
                 * filters are enabled, pressing this key should activate only
                 * the last one.
                 */

                if (buttonEnabledCounter > 0) {
                    /*
                     * If at least one button is enabled, get the index of the
                     * left-most enabled button, i.e. the first element in the
                     * list
                     */
                ...
                /*
                 * disable all the other buttons and notify the corresponding
                 * selection listener
                 */
                ...
                /*
                 * Activate only the button whose index is prevIndex and notify
                 * its selection listener
                 */
            ...
            case ']':
                ...
                /*
                 * If all the buttons are enabled, pressing the ']' key should
                 * de-activate all filters except the last one. If none of the
                 * filters are enabled, pressing this key should activate only
                 * the first one. If at least one button is enabled, pressing
                 * this key should activate the next filter button.
                 */
                if (buttonEnabledCounter > 0) {
                    pivotalIndex = enabledButtonIndexList
                            .get(indexArraySize - 1);
                ...
                /*
                 * If the nextIndex is greater than the length of the button
                 * array, decrement it
                 */
                ...
                /*
                 * Activate only the button whose index is nextIndex and notify
                 * its selection listener
                 */
            ...
            enabledButtonIndexList = new ArrayList<>(0);
        ...
        private void changeFcstHrLabelInGfaDialog() {
            ...
            if (indexOfHotKeyEnabledButton != -1) {
                currentFilter = new String(
                        hourBtns[indexOfHotKeyEnabledButton].getText());
            }
    ...
    private void updateTheGfaFcstHrIfItMatchesFilterHr(String currentFilter) {
        currentFilter = (currentFilter.endsWith("+")
                ? currentFilter.replace('+', ' ').trim() : currentFilter);
     ...
     * constructor
     */ /**
        * Creates a forecast hour filter dialog if the dialog does not exist and
            ...
            hourBtns[ii].addSelectionListener(new SelectionAdapter() {
                ...
                public void widgetSelected(SelectionEvent e) {
                    ...
                    if (((Button) e.widget).getSelection()) {
                        ...
                        drawingLayers.addFilter(filterMap.get(filterText));
                        ...
                        drawingLayers.removeFilter(filterMap.get(filterText));
    ...
    /***
     * Updates the Fcst Hr combo in the GfaAttrDlg (if open) depending on the
     * value of the filter hour(s) selected
     *
    ...
    private void updateGfaDialog(boolean activateButton, String filterText) {
        ...
        if (activateButton) {
            ...
            if (fcstCboTextArray != null && fcstCboTextArray.length > 0) {
                ...
                if (Integer.parseInt(filterText) > Integer
                        .parseInt(gfaFcstHour)) {
            ...
            for (int ii = HOURS.length - 1; ii >= 0; ii--) {
                ...
                if (thisBtnText.endsWith("+")) {
                    thisBtnText = new String(
                            thisBtnText.replace('+', ' ').trim());
                }
    ...
    protected void okPressed() {
        ...
        if (this.getButton(IDialogConstants.OK_ID).getText()
                .equalsIgnoreCase("All On")) {
            // add all filters

            for (Button btn : hourBtns) {
                btn.setSelection(true);
                drawingLayers.addFilter(filterMap.get(btn.getText()));
            }
            ...
            for (Button btn : hourBtns) {
                ...
                drawingLayers.removeFilter(filterMap.get(btn.getText()));
            }
            buttonEnabledCounter = 0;
    ...
    protected void cancelPressed() {
        ...
        drawingLayers.removeFilter(blockAll);
     ...
     * @param editor
     */
    public void setResource(PgenResourceList resources, AbstractEditor editor) {
        // public void setResource(PgenResource resource, NCMapEditor editor){
        this.drawingLayers = resources;
    ...
    public static Document readFilterHourTbl() {
        ...
        if (HOURS == null) {
            try {
                String filterHourFile = PgenStaticDataProvider.getProvider()
    ...
    private String[] getFilterHour() {
        if (filterHourTbl == null) {
            HOURS = new String[] { "0", "0+", "3", "3+", "6", "9", "12", "0-0",
                    "3-3", "6-6", "0-3", "0-6", "3-6", "6-9", "6-12", "9-12",
                    "AIRM", "OTLK" };
        } else {
    ...
    public void setHourChkBox(String hours, boolean flag) {
        ...
        // check if the input hour is in format H:MM, and then check if it is
        // between 0 and 3,
        if (hrBtn == null) {
            double fHrs = -1;
            double fMinutes = -1;
            ...
            if (fHrs > 0) {
                ...
                if (fHrs <= 0.001) {
                    hrStr = "0";
                } else if (fHrs > 0.001 && fHrs < 3) {
                    hrStr = "0+";
                } else if (Math.abs(fHrs - 3) < 0.001) {
                    hrStr = "3";
                } else if (fHrs > 3.001) {
                    hrStr = "3+";
                }
        ...
        if (hrBtn != null) {
            if (flag) {
                ...
                drawingLayers.addFilter(filterMap.get(hrBtn.getText()));
            } else {
                hrBtn.setSelection(false);
                drawingLayers.removeFilter(filterMap.get(hrBtn.getText()));
            }
        }
