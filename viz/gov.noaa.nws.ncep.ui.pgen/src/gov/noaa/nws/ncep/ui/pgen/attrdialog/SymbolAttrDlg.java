 ...
 * 04/09         90       B. Hebbard  Replace ColorSelector with
 *                                    ColorMatrixSelector.
 ...
 * 03/10         231      Archana     Altered the Symbol attribute dialog to
 *                                    display only a button showing the selected
 *                                    color instead of displaying the complete
 *                                    color matrix .
 * 11/10         ?        B. Yin      Set the dialog title to the
 *                                    PgenCategory(symbol/combo/marker)
 * 04/11         #?       B. Yin      Re-factor IAttribute
 * 05/12         756      B. Yin      Added a new method placeSymbol in order
 *                                    for ContourAttrDlg to override
...
public class SymbolAttrDlg extends AttrDlg implements ISymbol {
    ...
    protected static enum ChkBox {
        COLOR, CLEAR, WIDTH, SIZE, LAT, LON, LABEL
    };
    ...
    public double getSizeScale() {
        ...
        if (chkBox[ChkBox.SIZE.ordinal()].getSelection()) {
            return sizeSpinnerSlider.getSelection()
                    / Math.pow(10, sizeSpinnerSlider.getDigits());
        } else {
    ...
    private void setSize(double size) {
        sizeSpinnerSlider.setSelection(
                (int) (size * Math.pow(10, sizeSpinnerSlider.getDigits())));
    }
    ...
    /**
     * A Listener for lat/lon text fields to make sure input can only be
     * numbers, '-' or '.'
     *
     * @author bingfan
     *
    ...
    /**
     * A Listener class that implements ModifyListener to validate the latitude
     * text field.
     *
     * @author bingfan
     *
    ...
    public class LatModifyListener implements ModifyListener {
        ...
        public void modifyText(ModifyEvent e) {
            ...
            if (keyEvent) {
                ...
                /*
                 * If not key event, disable 'Place Symbol' button and 'Undo
                 * Symbol' button.
                 */
                placeBtn.setEnabled(false);
            }
    ...
    /**
     * A Listener class that implements ModifyListener to validate the longitude
     * text field.
     *
     * @author bingfan
     *
    ...
    /**
     * Update the symbol. Disable 'Place Symbol' button and 'Undo Symbol'
     * button.
     *
     */
    @Override
            ...
            for (AbstractDrawableComponent adc : adcList) {
                ...
                if (el != null) {
                    ...
                    if (latitudeText.isEnabled() && longitudeText.isEnabled()) {
                        ...
                        loc.add(new Coordinate(
                                Double.valueOf(longitudeText.getText()),
    ...
    private void createSizeAttr() {
        ...
        sizeSpinnerSlider = new gov.noaa.nws.ncep.ui.pgen.attrdialog.vaadialog.SpinnerSlider(
                inCmp, SWT.HORIZONTAL, 1);
        ...
        // default size
        sizeSpinnerSlider.setSelection(
                (int) (1 * Math.pow(10, sizeSpinnerSlider.getDigits())));
    ...
    private void createLatAttr() {
        ...
        latitudeText = new Text(latGroup, SWT.SINGLE | SWT.RIGHT | SWT.BORDER);
        ...
        undoBtn.addListener(SWT.MouseDown, new Listener() {
            ...
            public void handleEvent(Event event) {
                ...
                if (drawingLayers.getSelectedDE() != null) {
                    ...
                    drawingLayers.setSelected(
                            drawingLayers.getNearestElement(prevLoc));
    ...
    protected void placeSymbol() {
        ...
        DrawableElement elem = (DrawableElement) def.create(which,
                SymbolAttrDlg.this, pgenCategory, pgenType,
        ...
        if (drawingLayers.getSelectedDE() != null) {
            prevLoc = ((ISymbol) drawingLayers.getSelectedDE()).getLocation();
            drawingLayers.replaceElement(drawingLayers.getSelectedDE(), elem);
            ...
            PgenUtil.setDrawingTextMode(true,
                    ((LabeledSymbolAttrDlg) SymbolAttrDlg.this)
                            .useSymbolColor(),
                    defaultTxt, dec);
        } else {
