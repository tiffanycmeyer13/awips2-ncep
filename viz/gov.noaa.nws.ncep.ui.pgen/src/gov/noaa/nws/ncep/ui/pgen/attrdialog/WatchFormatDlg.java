 ...
 * 05/12         #776, 769  B. Yin    Added UTC time. Carry ove info from
 *                                    WCC/WCL dialogs.
...
public class WatchFormatDlg extends CaveJFACEDialog {
    ...
    // time zone buttons
    private Button estBtn, cstBtn, mstBtn, pstBtn, edtBtn, cdtBtn, mdtBtn,
            pdtBtn;
    ...
    public static String HOURS[] = { "0000", "0100", "0200", "0300", "0400",
    ...
    public static String HAILSIZE[] = { "0.5", "1.0", "1.5", "2.0", "2.5",
            "3.0", "3.5", "4.0" };
    ...
    public static String TOPLEVEL[] = { "300", "350", "400", "450", "500",
            "550", "600", "650", "700" };

    // motion combo, direction and speed
    private Combo dirCombo;

    public static String MOVEDIR[] = { "180", "190", "200", "210", "220", "230",
            "240", "250", "260", "270", "280", "290", "300", "310", "320",
            "330", "340", "350", "360" };

    private Combo spdCombo;

    public static String MOVESPD[] = { "10", "15", "20", "25", "30", "35", "40",
            "45", "50", "55", "60", "65", "70" };
     ...
     * @return
     */
    public static WatchFormatDlg getInstance(Shell parShell,
    ...
    protected void initializeComponents() {
        ...
        List<String> contWatch = PgenStaticDataProvider.getProvider()
                .loadContWatchNum();// loadContWatchNum();
        String cont = "";
    ...
    private Calendar getExpirationTime() {
        ...
        expiration.set(validDate.getYear(), validDate.getMonth(),
                validDate.getDay(), getExpHour(), getExpMinute(), 0);
        ...
        if (!err.isEmpty()) {
            MessageDialog infoDlg = new MessageDialog(
                    PlatformUI.getWorkbench().getActiveWorkbenchWindow()
                            .getShell(),
                    "Warning!", null, err, MessageDialog.INFORMATION,
                    new String[] { "OK" }, 0);
            infoDlg.open();
        } else {
            applyOnWB(wb);
            wbDlg.drawingLayers.resetElement(wb);
        ...
        contBtn.addSelectionListener(new SelectionListener() {
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                if (!(err.isEmpty())) {
                    ...
                    wbDlg.drawingLayers.resetElement(wb);
                    wbDlg.mapEditor.refresh();
                    msgDlg = new WatchFormatMsgDlg(
                            WatchFormatDlg.this.getParentShell(),
                            WatchFormatDlg.this);
    ...
    private void applyOnWB(WatchBox wb) {
        ...
        wb.setEndPointAnc(String.format("%1$s - %2$s",
                wb.getRelative(wb.getPoints().get(0), wb.getAnchors()[0]),
                wb.getRelative(wb.getPoints().get(4), wb.getAnchors()[1])));
        wb.setEndPointVor(String.format("%1$s - %2$s",
                wb.getRelative(wb.getPoints().get(0),
                        wb.getNearestVor(wb.getPoints().get(0))),
                wb.getRelative(wb.getPoints().get(4),
                        wb.getNearestVor(wb.getPoints().get(4)))));
        wb.setHalfWidthSm((int) Math.round(wb.getHalfWidth() / PgenUtil.SM2M));
        wb.setHalfWidthNm(
                (int) Math.round(wb.getHalfWidth() / PgenUtil.NM2M / 5.0) * 5);
    ...
    private void initDlg() {
        ...
        if (exp != null) {
            validDate.setDate(exp.get(Calendar.YEAR), exp.get(Calendar.MONTH),
                    exp.get(Calendar.DAY_OF_MONTH));
        ...
        if (wb.getWatchType() != null) {
            ...
            } else if (wb.getWatchType()
                    .equalsIgnoreCase("SEVERE THUNDERSTORM")) {
     ...
     */
    public static void setUTCTimeTextField(Composite parent,
            final Combo validTime, Calendar cal, Control topWidget,
            int offset) {

        validTime.setTextLimit(4);
        validTime.setText(
                String.format("%02d00", cal.get(Calendar.HOUR_OF_DAY)));
        validTime.addVerifyListener(new VerifyListener() {
            ...
            public void verifyText(VerifyEvent ve) {
                ...
                if (Character.isDigit(ve.character)
                        || Character.UNASSIGNED == ve.character
                        || ve.character == BACKSPACE
                        || ve.character == DELETE) {
                    ve.doit = true;
                } else {
                    ve.doit = false;
