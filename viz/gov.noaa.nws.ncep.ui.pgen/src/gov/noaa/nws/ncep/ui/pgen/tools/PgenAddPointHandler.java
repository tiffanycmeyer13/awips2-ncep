...
public class PgenAddPointHandler extends InputHandlerDefaultImpl {
    ...
    private final Symbol newDot = new Symbol(null,
            new Color[] { new Color(255, 0, 0) }, 2.5f, 7.5, false, null,
            "Marker", "DOT");
    ...
    public boolean handleMouseDown(int anX, int aY, int button) {
        ...
        if (button == 1) {
            ...
            case SELECTED:
                /*
                 * Create new element with new point added. Replace the old
                 * element in the resource with the new one.
                 */
                pgenrscs.removeGhostLine();
                AbstractDrawableComponent newComp = addPointToElement(loc,
                        pgenrscs.getSelectedComp());
    ...
    public boolean handleMouseMove(int anX, int aY) {
        ...
        case SELECTED:
            ...
            // ghost.add(newDot);
            newEl = addPointToElement(loc, pgenrscs.getSelectedDE());
    ...
    public boolean handleMouseDownMove(int x, int y, int mouseButton) {
        if (shiftDown || !tool.isResourceEditable()) {
            return false;
        } else {
            return preempt;
        }
     ...
     */
    public DrawableElement addPointToElement(Coordinate point,
            DrawableElement selectedDE) {

        DrawableElement newEl = (DrawableElement) selectedDE.copy();

        ArrayList<Coordinate> pts = calculateNewPoints(point,
                selectedDE.getPoints());
     ...
     */
    public AbstractDrawableComponent addPointToElement(Coordinate point,
            AbstractDrawableComponent elem) {
    ...
    /*
     * Adds the given point to the closest segment of the line defined by the
     * list of vertices
     */
    private ArrayList<Coordinate> calculateNewPoints(Coordinate point,
         ...
         */
        LineString ls = gf
                .createLineString(vertices.toArray(new Coordinate[] {}));
        ...
        if (loc.compareTo(lil.getStartIndex()) == 0) {
            /*
             * closest point is the first point. Add new point to beginning of
             * line
             */
            coords = new CoordinateList(new Coordinate[] { point }, false);
            coords.add(ls.getCoordinates(), false);
        } else if (loc.getSegmentIndex() == lil.getEndIndex()
                .getSegmentIndex()) {
            /*
             ...
             */
            LinearLocation previous = new LinearLocation(
                    loc.getComponentIndex(), loc.getSegmentIndex(), 0.0);
            LinearLocation next = new LinearLocation(loc.getComponentIndex(),
                    loc.getSegmentIndex() + 1, 0.0);
        ...
        for (int i = 0; i < coords.size(); i++) {
            pts.add(coords.getCoordinate(i));
        }
