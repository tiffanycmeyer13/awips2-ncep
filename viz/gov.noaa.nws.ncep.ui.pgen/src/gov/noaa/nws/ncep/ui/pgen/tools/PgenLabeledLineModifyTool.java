...
public class PgenLabeledLineModifyTool extends PgenSelectingTool
    ...
    private class PgenLabeledLineModifyHandler extends PgenSelectHandler {
        ...
        public boolean handleMouseMove(int x, int y) {
            ...
            if (attrDlg.isAddLineMode()) {
                ...
                if (labeledLine.getPgenType().equalsIgnoreCase("Cloud")) {
                    lineType = "SCALLOPED";
                } else if (labeledLine.getPgenType()
                        .equalsIgnoreCase("Turbulence")) {
                ...
                if (elSelected != null) {
                    if (elSelected.getParent() instanceof Label) {
                        ...
                        if (elSelected instanceof Line) {
                            ...
                            if (idx == 0) {
                                // select text
                                elSelected = ((Label) elSelected.getParent())
                                        .getSpe();
                            } else {
        ...
        public boolean handleMouseUp(int x, int y, int button) {
            ...
            if (button == 1 && drawingLayers != null) {
                ...
                if (elSelected != null) {
                    ...
                    if (elSelected instanceof SinglePointElement) {
                        // if the label is dropped on another label, merge them
                        // to one label.
                        if (elSelected.getParent() instanceof Label) {

                            mergedLine = PgenUtil.mergeLabels(newll,
                                    newll.getLabelAt(
                                            ((SinglePointElement) elSelected)
                                                    .getLocation()),
                                    ((SinglePointElement) elSelected)
                    ...
                    if (elSelected.getParent() instanceof Label) {
                        ...
                        if (elSelected instanceof Line && ghostEl != null
                                && ptSelected2) {
                            ptSelected2 = false;
                            // arrow line
                            ...
                            // should be label
                            if (elSelected.getParent() instanceof Label) {
                                // put original location back in order for undo
                                // to work
                                Label lbl = (Label) elSelected.getParent();
                    ...
                    } else if (elSelected instanceof Line && ghostEl != null
                            && ptSelected2) {
                        ptSelected2 = false;
                        // modify line
                        DrawableElement el = drawingLayers
                    ...
                    if ((labeledLine instanceof Ccfp)
                            && labeledLine.getLabels() != null
                            && labeledLine.getLabels().size() > 0) {
        ...
        private void addArrow(Coordinate loc, Label lbl) {
            ...
            Line aln = new Line(null, new Color[] { Color.WHITE }, 2.0F, 1.0,
                    false, false, locs, 0,
                    gov.noaa.nws.ncep.ui.pgen.display.FillPatternList.FillPattern.FILL_PATTERN_2,
                    "Lines", "POINTED_ARROW");
            lbl.addArrow(aln);
        }
