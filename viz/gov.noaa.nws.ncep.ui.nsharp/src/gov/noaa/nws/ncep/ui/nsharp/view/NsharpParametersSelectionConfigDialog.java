...
public class NsharpParametersSelectionConfigDialog extends Dialog {
    ...
    private boolean temp = true, dewp = true, parcel = true, parcelTv = true,
            dcape = true, vTemp = true, wetBulb = true, hodo = true,
            mixratio = false, dryAdiabat = true, moistAdiabat = false,
            omega = true, meanWind = true, smv3075 = false, smv1585 = false,
    ...
    private void createDialogContents(Composite parent) {
        ...
        parcelTvBtn.setBounds(btnGp.getBounds().x + btnGapX,
                dewpBtn.getBounds().y + dewpBtn.getBounds().height + btnGapY,
                btnWidth, btnHeight);
        if (parcelTv == true)
        ...
        vTempBtn.setBounds(btnGp.getBounds().x + btnGapX,
                dcapeBtn.getBounds().y + dcapeBtn.getBounds().height + btnGapY,
                btnWidth, btnHeight);
        if (vTemp == true)
        ...
        wetBulbBtn.setBounds(btnGp.getBounds().x + btnGapX,
                vTempBtn.getBounds().y + vTempBtn.getBounds().height + btnGapY,
                btnWidth, btnHeight);
        if (wetBulb == true)
        ...
        meanWindVectorBtn.setBounds(btnGp.getBounds().x + btnGapX,
                hodoBtn.getBounds().y + hodoBtn.getBounds().height + btnGapY,
                btnWidth, btnHeight);
        if (meanWind == true)
        ...
        stormMVector3075Btn
                .setText(NsharpNativeConstants.STORM_MOTION_VECTOR_3075);
        stormMVector3075Btn.setEnabled(true);
        stormMVector3075Btn.setBounds(btnGp.getBounds().x + btnGapX,
                meanWindVectorBtn.getBounds().y
                        + meanWindVectorBtn.getBounds().height + btnGapY,
                btnWidth, btnHeight);
        if (smv3075 == true)
        ...
        stormMVectorBunkersLeftBtn
                .setText(NsharpNativeConstants.STORM_MOTION_VECTOR_BUNKERS_L);
        ...
        windBarbBtn.setBounds(btnGp.getBounds().x + btnGapX,
                corfidiVectorBtn.getBounds().y
                        + corfidiVectorBtn.getBounds().height + btnGapY,
                btnWidth, btnHeight);
        if (windBarb == true)
        ...
        tempOffsetText.setBounds(
                tempOffsetLbl.getBounds().x + tempOffsetLbl.getBounds().width,
                tempOffsetLbl.getBounds().y, btnWidth / 4, btnHeight);
        ...
        // to make sure user enter digits only
        sndCompRadiusText.addListener(SWT.Verify, new Listener() {
            public void handleEvent(Event e) {
                String string = e.text;
                char[] chars = new char[string.length()];
                string.getChars(0, chars.length, chars, 0);
    ...
    public void okPressed() {
        ...
        textStr = sndCompRadiusText.getText();
        if ((textStr != null) && !(textStr.isEmpty())) {
            sndCompRadius = Integer.decode(textStr);
            System.out.println("snd comp radius is =" + sndCompRadius);
        }
        applyChange();
        setReturnCode(OK);
    }
