...
import gov.noaa.nws.ncep.edex.plugin.airep.AirepSeparator;
...
public class AirepParser {
    ...
    private static final String DATE_FORMAT_PATTERN = "yyyyMMddHH";
    ...
    private ArrayList<Object> theElements = new ArrayList<>();
    ...
    ArrayList<?> parseElementsTestPoint() {
        ArrayList<Object> retElements = new ArrayList<>();
    ...
    private void parseElements() {
        ...
        headerDate = getHeaderDate(AirepSeparator.getHeaderDate());
        ...
        if ((latitude == null) && (longitude == null)) {
            for (int i = 0; i < theElements.size(); i++) {
                ...
                if (o instanceof String) {
                    BasePoint wayPoint = PlatformLocationProxy
                            .lookup((String) o, null);
                    // found a waypoint
                    if (wayPoint != null) {
    ...
    private void splitLatLon() {
        ...
        for (int i = 0; i < theElements.size(); i++) {
            ...
            if (o != null && o instanceof String) {
                String s = theElements.get(i).toString().trim(); // e.g. UAE412
                                                                 // 3606S 15942E
                                                                 // 1150 F410
                                                                 // MS47 281/75=

                if (!s.equalsIgnoreCase("")) {
                    String[] split = s.split("[A-Z]"); // this can get 3606S,
                                                       // 15942E, 1150, 281/75,
                                                       // so need return.
                    ...
                    if (split.length != 0) {
                        ...
                        if (splitPos != 0) { // not start with a letter
                            if (splitPos < s.length() - 1) { // s is like
                                                             // 3606S15942E, no
                                                             // space
    ...
    private void decodeTime() {
        // Current Day - insert date
        Calendar oTime = TimeUtil.newGmtCalendar();
        if (headerDate != null) {
            oTime.setTime(headerDate);
        }

        // Copy the oTime to issuTime
        Calendar issuTime = copy(oTime);
        ...
        // if observed day is much bigger than current day
            ...
            if (o instanceof String) {
                ...
                if (TIME_Airep.matcher(s).matches()) {
                    ...
                    if (observationTime.getTime() != issuTime.getTime()) {
                        observationTime = checkDayInTime(observationTime,
                                issuTime);
                    }
                    logger.info("AREP MATCH***********time: "
                    ...
                    int eventDay = Integer.parseInt(s.substring(0, 2));
                    ...
                    // If header dayOfMonth = 1 and day = "Last day of previous
                    // month"
                    if (day == 1) {
                        Calendar dateMath = copy(oTime);
                        dateMath.add(Calendar.DAY_OF_MONTH, -1);
                        int lastDayPrevMonth = dateMath
                                .get(Calendar.DAY_OF_MONTH);
                        if (eventDay == lastDayPrevMonth) {
                            logger.info("Header day is 1 and eventDate is "
                                    + lastDayPrevMonth);
                            observationTime.setTime(dateMath.getTime());
                        }
    ...
    /**
     * Decode the temperature information in this observation. e.g. Airep MS45,
     * AMDAR MS456
     */
    ...
    /*
     * Observed hazard in TB, SK, IC order in Airep. Observed appear once at
     * most for each. e.g. airep: ARP ASA858 3948N 13807W 2354 F370 MS53
     * 263/046KT TB SMTH= amdar: AMDAR LVR AU0144 2903S 15252E 202133 F350 MS440
     * 193/081 TB3 S031
     *
     */
    private void decodeHazard() {
        ...
        if (iTb != 0 && iTb < size) { // iTb !=0 eliminate header
            if (!theElements.get(iTb).toString().equalsIgnoreCase("SK")
                ...
                if (temp.startsWith("CO") || temp.startsWith("OC")
                        || temp.startsWith("INT")) {
                    turbFreq = temp;
        ...
        if (iIc != 0 && iIc < size) {
            ...
            if (!theElements.get(iIc).toString().equalsIgnoreCase("RM")
                    && !theElements.get(iIc).toString()
                            .equalsIgnoreCase("SK")) { // didn't see, in case...
                ...
                if (temp.startsWith("RIM") || temp.startsWith("CL")
                        || temp.startsWith("MX")) {
                    iceType = temp;
                } else {
                    iceInten = temp;
    ...
    public static void main(String[] args) {
        ...
        ArrayList<Object> theElements = new ArrayList<>();
    ...
    static final Calendar copy(Calendar calendar) {
