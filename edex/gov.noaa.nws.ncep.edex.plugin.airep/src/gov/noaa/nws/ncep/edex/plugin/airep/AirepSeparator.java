 ...
 * Apr 14, 2022 95500      thuggins    Pulling the header date string from com.raytheon.edex.esb.Headers
 *                                     and allowing it to be passed along to AirepParser
 * </pre>
 *
 * @author F. J. Yen
 * @version 1.0
...
public class AirepSeparator extends AbstractRecordSeparator {
    ...
    private static String headerDate = null;
    ...
    public static AirepSeparator separate(byte[] data, Headers headers) {
        ...
        headerDate = getHeaderDateStr(headers);
    ...
    /**
     * Separate amdar data that comes in with AMDAR header line. 097 ^M UDAS02
     * BABJ 190113^M AMDAR 1901^M LVR CNFNXL 3315N 11850E 190113 F226 MS123
     * 278/038 TB/ S//1=^M
     */
    ...
    /**
     * Separate airep data that comes in with AIREP header line. 968 ^M UAFJ01
     * NFFN 200000^M AIREP^M JST4 0050N17838W 2319 F360 MS46 302/025=^M
     */
     ...
     * is checked to ensure that it contains ARP/ARS report start data. The
     * ending of the ARP/ARS report could be "=", or only has one ARP/ARS
     * report. 706 ^M UAPA01 KWBC 200000^M ARP UAL559 3401N 13421W 2351 F360
     * MS52 260/025KT=
     */
    private void separateARPARS(String message) {
        ...
        // find body msg
        ArrayList<Integer> bodyRecords = new ArrayList<>();
        ...
        for (int i = 0; i < bodyRecords.size(); i++) {
            ...
            if (i < bodyRecords.size() - 1) {
                observation = message
                        .substring(bodyRecords.get(i), bodyRecords.get(i + 1))
                        .trim();
            } else {
     ...
     * @return
     */
    static String getHeaderDateStr(Headers headers) {
        String headerDateStr = null;
        if (headers != null) {
            Object o = headers.get("CamelFileNameOnly");
            if (o != null) {
                String file = o.toString();
                int lastIndex = file.lastIndexOf(".");
                if (lastIndex != -1) {
                    headerDateStr = file.substring(lastIndex + 1);
                }
            }
        }
